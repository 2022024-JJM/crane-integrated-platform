...
export interface SavedModelInfo {
  ...
  /**
   * 영역 감지에서 이 모델을 통째로 제외한다 — 다른 모델의 영역에 들어가도
   * 침범으로 세지 않고, **이 모델의 `zones` 도 감지하지 않으며 링도 그리지
   * 않는다**(인스펙터 목록도 비활성). 정의는 남으므로 제외를 풀면 그대로
   * 되살아난다. 고정 설비·지형 장식처럼 감지에 끼지 않아야 하는 모델용.
   * locked 과 같은 true-only 직렬화.
   */
  zoneExempt?: boolean;
}
