 ...
 *   (markSceneOpaqueStencil). 바다(features lib/ocean-water.ts · ui/scene-water
 *   .tsx)가 불투명 패스 **뒤에** 이 비트가 없는 픽셀에서만 그려진다. 바다는
 *   깊이를 쓰지 않으므로(수면 아래 드라이독·잠긴 선체를 가리면 안 된다)
 *   "무엇이든 그려졌으면 진다" 는 규칙을 깊이가 아니라 스텐실로 만든다.
 *   스텐실 테스트는 프래그먼트 셰이더 앞(early test)에서 끝나므로 가려진
 *   픽셀은 바다 셰이더(노멀맵 4회 + 미러 샘플) 비용이 0 이다.
 *
