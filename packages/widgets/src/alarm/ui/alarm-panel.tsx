...
import type { Alarm, AlarmSeverity, AlarmStatistics } from '@crane/domain/alarm';
import { getAlarmSeverityLabel } from '@crane/domain/alarm';
import { Select, SelectItem, SelectPopup, SelectTrigger } from '@crane/ui/molecules/select';
...
const SEVERITY_VALUES: SeverityFilter[] = ['all', 'critical', 'high', 'medium', 'info'];
...
export function AlarmPanel({ stats, alarms }: AlarmPanelProps) {
  ...
  const filteredAlarms = filter === 'all' ? alarms : alarms.filter((a) => a.severity === filter);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b px-3 py-1.5">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[11px] text-muted-foreground">{t("common:alarms.alarmStatus")}</span>
          <Select value={filter} onValueChange={(v: string) => setFilter(v as SeverityFilter)}>
