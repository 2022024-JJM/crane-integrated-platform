 ...
 *
 * 기존 `SceneCollisionAlertOverlay` 를 고치지 않고 형제로 둔 이유는 그
 * 컴포넌트가 활성 기록이 없으면 곧바로 null 을 돌려주기 때문이다 — 예측
 * 단독 상태를 표현할 수 없다.
 *
 * **가장자리 비네트는 두지 않는다**(2026-09-10 제거). 실제 충돌은 즉시
 * 개입해야 하는 일회성 사건이라 화면 전체를 물들일 값어치가 있지만, 예측은
 * 재생 내내 떴다 사라지기를 반복하는 지속 상태다. 그 배경을 계속 물들이면
 * 정작 봐야 할 고스트와 씬이 가려지고, 빨간 충돌 비네트가 함께 뜰 때는 두
 * 농도가 겹쳐 어느 쪽도 안 읽힌다. 상단 배너와 씬 안 카운트다운만으로
 * 충분하다.
 *
