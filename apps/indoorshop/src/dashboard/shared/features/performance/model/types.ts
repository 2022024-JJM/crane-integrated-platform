 ...
 *   가공 = [적치] S1 강재 입고 → S2 1차 선별 → S3 2차 선별 → S4 강재 불출 →
 *          [가공] S5 가공 입고 → S6 전처리 → S7 절단 → S8 사상 → S9 모둠선별 →
 *                 S10 최종 불출 (**정본 10절점**, 두 단계로 묶인다)
...
/**
 * 가공권역 **정본 10절점** (사용자 확정 — R33 → **R39 정정**).
 *
 * 흐름 그대로다. 절점은 두 단계로 묶인다 — **S4 까지가 적치**고 그 다음이 **가공**이다:
 *   [적치] 강재 입고 → 1차 선별 → 2차 선별 → **강재 불출**
 *   [가공] **가공 입고** → 전처리 → 절단 → 사상 → **모둠선별** → 최종 불출
 *
 * R39 에서 실창 검토로 셋이 바뀌었다:
 *  - **'가공 입고' 신설** — 불출(적치의 끝)과 전처리(가공의 시작) 사이에 실제로 있는 절점이다.
 *    두 단계의 경계가 여기 있으므로 이 절점이 없으면 적치/가공이 어디서 갈리는지 화면이 못 말한다.
 *  - **'팔레트 편성' → '모둠선별'** — 현업 어휘가 그렇다. 같은 절점의 이름만 바뀐 것이라
 *    판별 근거(③④ 모듬 · ⑤ 선별)는 그대로 따라간다.
 *  - **'변성' 제거** — 절점이 아니다(사용자 지시 "일단은 변성 단계는 빼").
 *
 * 이전 구현은 이 축의 축약본(5절점)이었다. 축약의 근거는 "선별·전처리는 원천에 완료
 * 근거가 없다" 였는데, **절점 축과 원천 확정 여부는 다른 문제**다 — 축은 공정이 정하고,
 * 원천은 뒤따라 붙는다. 그래서 축을 정본으로 세우고, 근거가 아직 없는 절점은 화면이
 * '원천 확정 대기' 로 그 사정을 적는다(`FAB_STAGES_PENDING_SOURCE`).
 *
 * ⚠️ **S4 는 '강재 불출'** 이다. 정의서의 `전처리(불출)` 표기와 달리 판별 근거 필드가
 * `강재불출.불출일자` 이고, 실제 흐름에서도 불출이 전처리(S6)보다 앞선다 — 둘은 별개 절점이다.
 */
...
/**
 * 가공 절점의 **큰 묶음** (사용자 확정 R39 — "s4까지는 적치 단계야 그다음이 가공 단계고").
 *
 * 열 칸을 한 줄로만 늘어놓으면 어디까지가 강재를 쌓아 두는 일이고 어디부터가 실제 가공인지
 * 읽히지 않는다. 화면(절점 스트립·절점 카드)은 이 묶음으로 두 덩이를 갈라 보여 준다.
 */
export type FabStageGroup = 'stack' | 'fab'

export const FAB_STAGE_GROUPS: readonly FabStageGroup[] = ['stack', 'fab']

export const FAB_STAGE_GROUP: Record<FabStageId, FabStageGroup> = {
  S1: 'stack',
  S2: 'stack',
  S3: 'stack',
  S4: 'stack',
...
/** 그 묶음에 속한 절점 — 순서는 `FAB_STAGES` 를 따른다 */
export function fabStagesOfGroup(group: FabStageGroup): FabStageId[] {
  return FAB_STAGES.filter((stage) => FAB_STAGE_GROUP[stage] === group)
}

/**
 * 원천(레거시 컬럼)이 아직 확정되지 않은 절점 — 화면이 '원천 확정 대기' 배지를 세운다.
 *
 * 절점 축은 정본이지만 판별 근거 필드가 확정된 것은 다섯(S1 강재 입고·S4 강재 불출·
 * S7 절단·S8 사상·S9 모둠선별)뿐이다. R39 로 번호가 밀렸어도 **근거는 절점을 따라간다** —
 * 신설된 '가공 입고'(S5)는 아직 근거가 없어 대기 쪽이다. 이 사실을 감추면 화면이
 * "다 수집되고 있다" 는 거짓말을 한다 — 그래서 타입 옆에 함께 둔다.
 */
export const FAB_STAGES_PENDING_SOURCE: readonly FabStageId[] = ['S2', 'S3', 'S5', 'S6', 'S10']
 ...
 * (`강재불출.불출일+시각`)과 **S7 절단**(`절단완료일시`) 둘뿐이고, 나머지 여덟은 일자만이다.
 * 원칙은 절점 재편(R33·R39)을 거치며 그대로고, 따라가는 것은 번호뿐이다.
 */
export interface EventInstant {
  date: string
  /** HH:mm — S4(강재 불출)·S7(절단) 만 존재 */
  time?: string
}
