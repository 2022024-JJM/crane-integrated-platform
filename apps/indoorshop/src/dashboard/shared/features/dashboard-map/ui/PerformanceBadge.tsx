 ...
 *
 * 나가는 문은 둘이다. 종합(위쪽 큰 수치)은 통합실적 화면만 열고, **블록 한 줄은 그
 * 호선·블록을 실어 보낸다**(`?vessel=&block=`) — 눌러 놓고 도착해서 같은 블록을 다시
 * 고르는 일이 없게 한다. 종합에 조건을 싣지 않는 것은 한 공장에 여러 호선의 블록이
 * 서기 때문이다 — 통합실적의 조회 단위는 호선 하나라 공장 전체를 한 조건으로 말할 수 없다.
 */
...
async function loadFactoryPerf(factory: string): Promise<FactoryPerf | null> {
  const api = await import('../../performance/api/performanceApi')
  ...
  const rows = await Promise.all(
    pairs.map(async ({ projNo, blockNo }): Promise<BlockPerfRow> => {
      ...
      return {
        projNo,
        blockNo,
        recognizedQty: summary.recognizedQty,
        reqQtyTotal: summary.reqQtyTotal,
        rate: summary.judgedRate,
      }
    })
  )
  const recognizedQty = rows.reduce((s, r) => s + r.recognizedQty, 0)
  const reqQtyTotal = rows.reduce((s, r) => s + r.reqQtyTotal, 0)
  return {
    rows,
    recognizedQty,
    reqQtyTotal,
  ...
  return (
    <div className={cn('shrink-0 border-b border-white/8 px-3 py-3', className)}>
      <div className="rounded-inshop-lg border border-white/10 bg-white/[0.03] px-3 py-2.5">
        ...
        {/* 블록별 한 줄 — 통합실적 헤더 카드의 W/O 수와 같은 숫자다(같은 로스터·같은 기준일).
            누르면 그 블록이 골라진 채로 통합실적이 열린다. */}
        <ul className="mt-2 space-y-0.5">
          {rows.map((row) => (
            <li key={`${row.projNo}-${row.blockNo}`}>
              <Link
                to={performanceLinkFor({ projNo: row.projNo, blocks: [row.blockNo] })}
                title={t('dashboard.map.perfBadge.blockHint', {
                  block: `${row.projNo}-${row.blockNo}`,
                })}
                className="flex items-center gap-2 rounded px-1 py-0.5 text-2xs text-white/60 transition-colors hover:bg-white/8 hover:text-white/85 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70"
              >
                <span className="font-mono tabular-nums text-white/78">
                  {row.projNo}-{row.blockNo}
                </span>
                <span className="ml-auto font-mono tabular-nums">
                  {row.recognizedQty}/{row.reqQtyTotal}
                </span>
