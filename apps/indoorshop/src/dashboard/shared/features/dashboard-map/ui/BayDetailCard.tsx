...
import { PerformanceLink, blockAtBay } from '../../../entities/vessel'
...
import { toggleSpottedLot } from '../lib/lotSpot'
 ...
 *
 * 그 베이에 블록이 서 있으면 나가는 문이 하나 더 붙는다 — **통합실적**. 공정 상세는
 * "이 정반에서 지금 무엇을 하고 있나"를, 통합실적은 "그 블록이 어디까지 왔나"를
 * 말하므로 서로를 대신하지 못한다. 로스터가 정반↔블록을 알기에 여기서 이을 수 있다
 * (블록이 없는 베이·mock 우주 밖의 베이에는 링크가 서지 않는다 — `PerformanceLink` 가
 * 스스로 거른다).
 *
      ...
      <div className="shrink-0 border-t border-white/8 px-4 py-3">
        {linkedLocation ? (
          <>
            ...
            {/* 이 정반에 선 블록의 실적으로 — 공정 상세와 다른 질문에 답하는 문이다 */}
            {(() => {
              const block = blockAtBay(linkedLocation.id)
              return block ? (
