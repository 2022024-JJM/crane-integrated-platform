...
describe('zoneJumpHref — 공정 화면으로 넘어가는 문', () => {
  it('맵 화면이 있는 세 공정은 그 공장을 연 주소를 낸다 — 값은 안정 슬러그(F-30)', () => {
    expect(zoneJumpHref('조립', 'GBS')).toBe('/zones/assembly?factory=asm-gbs')
    expect(zoneJumpHref('의장', 'POS 1공장')).toBe('/zones/outfitting?factory=ofit-pos1')
    expect(zoneJumpHref('도장', '1DOCK 도장공장')).toBe('/zones/painting?factory=pnt-1dock')
