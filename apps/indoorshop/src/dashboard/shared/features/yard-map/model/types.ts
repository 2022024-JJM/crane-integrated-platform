/**
 * 야드 맵이 그리는 것들의 좌표·도메인 타입 — BTS(블록운반시스템) 야드 도메인.
 *
 ...
 *
 * 이 파일은 `shared/features/yard-map` 의 **프레젠테이션 계약**이다 — 지도는 여기 정의된
 * 순수 타입·기하 함수만 알고, 이 값들을 어디서 채우는지(fixture·API)는 알지 않는다.
 */
...
export function boundsOf(points: readonly LatLon[]): LatLonBounds {
  let minLat = Infinity
  let minLon = Infinity
  let maxLat = -Infinity
  let maxLon = -Infinity
