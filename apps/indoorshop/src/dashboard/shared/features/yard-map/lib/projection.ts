import type { LatLonBounds } from '../model/types'
 ...
 *
 * 중심(위경도)·기울기·방위는 선형으로, **배율은 로그 공간에서** 섞는다: 확대는
 * 곱으로 느껴지는 양이라, 선형으로 섞으면 가까울수록 급하고 멀수록 굼떠 보인다.
 */
export function lerpView(from: YardView, to: YardView, e: number): YardView {
  return {
    centerLat: from.centerLat + (to.centerLat - from.centerLat) * e,
    centerLon: from.centerLon + (to.centerLon - from.centerLon) * e,
