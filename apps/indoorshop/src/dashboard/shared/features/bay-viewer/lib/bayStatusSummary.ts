 ...
 * 정반 라벨의 작업 줄 문구 키.
 *
 * 실측 정반(PBS 5BAY)은 '작업 중 N건' 대신 **실측임을 앞세운다** — 여기의 N 은 목업
 * 판별 건수가 아니라 실측 정합 블록 수이고, 공장 뷰는 그 형상을 그리지 않으므로
 * (좌표 프레임이 달라 그릴 수 없다 — R 진단) 사용자를 정반 뷰로 이끌어야 한다.
 * 순수 함수로 두는 이유는 이 레포의 다른 파생 계산과 같다: 문구 규칙이 DOM 생성
 * 안에 있으면 검증할 수 없다.
 */
export function bayWorkLineKey(
  workState: BayWorkState,
  realScan: boolean
):
  | 'viewer.bayStatus.realScanWorking'
  | 'viewer.bayStatus.realScanIdle'
  | 'viewer.bayStatus.working'
  | 'viewer.bayStatus.idle'
  | 'viewer.bayStatus.noData' {
  if (realScan) {
    /* 자산 미로드 등으로 블록 수를 모르더라도 실측 정반임은 변하지 않는다 */
    return workState === 'working' ? 'viewer.bayStatus.realScanWorking' : 'viewer.bayStatus.realScanIdle'
  }
  if (workState === 'working') return 'viewer.bayStatus.working'
  if (workState === 'idle') return 'viewer.bayStatus.idle'
  return 'viewer.bayStatus.noData'
}
