...
import { bayWorkLineKey, type BayWorkState, type SensorStatusCounts } from './bayStatusSummary'
...
> = {
  online: {
    ...
    labelKey: 'viewer.sensorStatus.online',
  },
  offline: {
    dot: 'bg-glass-foreground/40',
    ink: 'text-glass-foreground/63',
    labelKey: 'viewer.sensorStatus.offline',
  },
  error: {
    dot: 'bg-glass-unhealthy',
    ink: 'text-glass-unhealthy',
    labelKey: 'viewer.sensorStatus.error',
  },
  calibrating: {
    dot: 'bg-glass-degraded',
    ink: 'text-glass-degraded',
    labelKey: 'viewer.sensorStatus.calibrating',
  },
}
...
export interface BayStatusLabelData {
  ...
  /**
   * 실측 스캔 정반(PBS 5BAY) — 이름 옆 실측 칩을 달고, 작업 줄을 '실측 스캔 ·
   * 블록 N — 진입해 확인'으로 바꾼다. 공장 뷰는 실측 형상을 그리지 않으므로
   * (좌표 프레임 비정합) 라벨이 그 사실과 진입 경로를 대신 말해야 한다.
   */
  realScan?: boolean
}
...
export function createBayStatusLabel(
  ...
  const workText = t(bayWorkLineKey(data.workState, data.realScan ?? false), {
  ...
  wrap.innerHTML = `
    <button type="button" title="${summary}" class="flex flex-col items-stretch gap-0.5 whitespace-nowrap rounded-md glass-panel px-2 py-1 text-left transition-colors hover:bg-glass-hover focus:outline-none focus-visible:ring-2 focus-visible:ring-glass-accent${
      ...
      <span class="flex items-center gap-1.5">
        ...
        ${
          data.realScan
            ? `<span class="rounded-sm bg-glass-accent/20 px-1 text-2xs font-medium text-glass-accent">${t('viewer.bayStatus.realScanBadge')}</span>`
            : ''
        }
      </span>
      ${
