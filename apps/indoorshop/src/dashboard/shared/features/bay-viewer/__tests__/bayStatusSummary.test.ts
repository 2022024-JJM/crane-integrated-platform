...
describe('bayWorkLineKey — 실측 정반은 실측임을 앞세운다 (R 진단 (b))', () => {
  it('실측 + 작업 중 → 블록 수를 실측 문구로', () => {
    expect(bayWorkLineKey('working', true)).toBe('viewer.bayStatus.realScanWorking')
  })
  it('실측 + 자산 미로드(idle) 라도 실측 표기는 유지된다', () => {
    expect(bayWorkLineKey('idle', true)).toBe('viewer.bayStatus.realScanIdle')
  })
  it('목업 정반은 기존 문구 그대로', () => {
    expect(bayWorkLineKey('working', false)).toBe('viewer.bayStatus.working')
    expect(bayWorkLineKey('idle', false)).toBe('viewer.bayStatus.idle')
    expect(bayWorkLineKey('noData', false)).toBe('viewer.bayStatus.noData')
  })
})
