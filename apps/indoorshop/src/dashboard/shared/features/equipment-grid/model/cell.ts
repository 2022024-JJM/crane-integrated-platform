...
export interface EquipmentLamp {
  ...
  value?: string
  /**
   * 값을 화면에도 적을 것인가 — **값 자체가 정보인** 램프만(`소속 10/12`).
   *
   * 대부분의 램프에서 값은 색이 이미 말한 것을 되풀이하는 말이라(`링크 online`) 적으면
   * 소음이 된다. 그런데 수량은 다르다: 램프 색은 "몇 대가 빠졌나"를 말하지 못한다.
   * 이름 붙은 램프(`namedLamps`)에서만 선다 — 익명 점 옆의 숫자는 무엇의 수인지 모른다.
   */
  showValue?: boolean
}
...
export interface EquipmentCell {
  ...
  metric: {
    ...
    meaning: StatusMeaning
    /**
     * 이 값을 마지막으로 받은 시각(epoch ms).
     *
     * 주면 셀이 **살아 있는 경과**("12초 전")를 스스로 흘려 보내고, 침묵이 길어지면 그
     * 사실을 드러낸다(R19 실시간감). 주지 않으면 `text` 를 그대로 적는다 — 시각이 없는
     * 값(설정값·대수)까지 억지로 흐르게 만들지 않는다.
     */
    at?: number
  }
  /**
  ...
  /**
   * 램프 자리에 서는 **그림 한 개** — 뜻이 본래 그림인 값(조준 방향 등)을 낼 때.
   *
   * 주면 그 셀의 램프 **점은 그리지 않는다**: 그림이 이미 그 사실을 말하고 있는데
   * 점을 함께 세우면 같은 것을 두 번 말하면서 줄만 좁아진다. 다만 `lamps` 배열은
   * 그대로 채워 둔다 — 지도 카드(`BirdviewCard`)와 보조기술이 그 배열을 읽는다.
   * 그림 스스로 접근성 이름을 들어야 한다(`role="img"` + `aria-label`).
   *
   * 안 주면 지금까지 그대로 램프 점이 선다 — 의장·도장 셀은 손대지 않는다.
   */
  figure?: ReactNode
