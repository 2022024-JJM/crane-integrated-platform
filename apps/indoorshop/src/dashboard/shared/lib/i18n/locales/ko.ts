...
export const ko = {
  ...
  dashboard: {
    ...
    map: {
      ...
      perfBadge: {
        refChip: '참고',
        title: '절점 기반 실적 (통합실적과 동일 원천)',
        woCount: 'W/O {{done}}/{{total}} 완료',
        note: '절점 W/O 완료 기준 참고 수치 — 공식 실적 지표가 아닙니다. 누르면 통합실적으로 이동합니다.',
        openHint: '통합실적 화면 열기',
      },
  ...
  performance: {
    ...
    asm: {
      title: '조립 — 블록·ASSY 실적',
      woBasis: 'ASSY 기준 추적 · % 는 W/O 완료 기준',
      gridStage: '조립',
      assyDone: 'ASSY 완료',
      overall: '종합 W/O 완료',
      overallNote: 'ASSY 귀속 W/O 완료 합계 — 합성 산식 아님',
      counting: '카운팅 (분모 REQ_QTY)',
      countingPending: '인식 카운트는 OT 자동수집 가동 후 실값 — 지금은 W/O 상태 파생',
      assyStatus: { done: '완료', inProgress: '진행중', notStarted: '미착수' },
      tier: { grand: '대조', mid: '중조', sub: '소조' },
      treeSummary: 'ASSY {{done}}/{{total}} 완료',
    ...
    pnt: {
      ...
      provisionalNote:
        '추정 명세(SE12 검증 전) — 스텝↔레거시 키 매핑은 잠정(paintingStepMapping.ts 한 곳)',
