...
> ⚠️ **LiDAR 대수 출처 불일치.** 본 절은 조립 `340대`(물리 배포 다이어그램 근거)로 적혀 있으나, 「필드 인터페이스 정의서」는 **조립 ~210대 + 의장 ~140대(합 ~350대)** 로 기술한다. 총량은 비슷하나 **권역별 배분이 다르므로** 어느 쪽이 최신인지 확인이 필요하다. 위 표는 후자 기준으로 적었다.
