...
export function OutfittingDeviceStatusList({
  ...
  const { t } = useTranslation()
  // 경과 표기가 굳지 않도록 30초마다 다시 계산한다 (조립 목록과 같은 주기)
    ...
    >
      ...
      <ul className="divide-y divide-border/50 px-1.5 py-1">
        {devices.map((device) => {
          ...
          const failing = device.status !== 'online'
          return (
            <li
              key={device.id}
              className={cn('rounded px-1.5 py-1', failing && 'ring-1 ring-inset', failing && STATUS_RING[device.status])}
            >
