...
export const assemblyModule: ProcessModule = {
  ...
  routes: [
    /* 엔트리는 **워크스페이스**다 (R22) — 공정별 맵 진입 화면을 걷었다. 공장 없이
     * 들어오면 워크스페이스가 `?factory=`(총괄 점프)나 첫 공장을 편다.
     * 고정 경로 `list` 는 `:factoryId` 보다 먼저 서야 한다(뒤에 두면 공장 하나로 잡힌다). */
    { path: '/indoorshop/zones/assembly', Component: AssemblyWorkspace },
  ...
  provides: {
    ...
    factoryOverviews: (baseDate) =>
      import('./api/assemblyApi').then((m) => m.fetchFactoryOverviews(baseDate)),
