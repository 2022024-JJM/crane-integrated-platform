...
/* 유리 위에서는 상태색도 어두운 바탕용으로 바꿔 잡는다 (globals.css 의 --glass-* 계열) */
const glassStatusDot: Record<LocationStatus, string> = {
  occupied: 'bg-glass-healthy',
  empty: 'bg-glass-foreground/30',
  unknown: 'bg-glass-degraded',
}
...
export function AssemblyLocationTabs({
  ...
  const dot = glass ? glassStatusDot : statusDot
            ...
            >
              <span
                ...
                className={cn(
                  ...
                  isActive && !glass ? 'bg-on-accent/70' : dot[bay.status],
                )}
              />
