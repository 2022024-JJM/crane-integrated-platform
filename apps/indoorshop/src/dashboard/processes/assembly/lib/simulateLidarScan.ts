 ...
 * slab 테스트의 tMax가 곧 그 거리다.
 */
function rayBayExitDistance(origin: THREE.Vector3, dir: THREE.Vector3): number {
  let tMax = Infinity
  const axes: [number, number, number, number][] = [
    [origin.x, dir.x, -BAY_WIDTH / 2, BAY_WIDTH / 2],
    [origin.y, dir.y, 0, BAY_HEIGHT],
    [origin.z, dir.z, -BAY_LENGTH / 2, BAY_LENGTH / 2],
  ]
...
export function simulateBaySurfaceScan(
  ...
  for (let i = 0; i < rayCount; i++) {
    ...
    const tBay = rayBayExitDistance(sensorPosition, dir)
