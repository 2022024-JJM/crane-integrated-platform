...
|------|----------------|----------------|
| 센서 | RFID, LiDAR | PLC(Modbus) |
| 대표 이벤트 | `RFID_TAG_READ`, `LIDAR_BLOCK_DETECTED` | `PLC_COIL_ON`, `PLC_COIL_OFF` |
...
| Port | 조립 | 도장 |
|------|------|------|
| Inbound | ISL4 Provider → RFID/LiDAR를 `ProcessEvent`로 변환 | Modbus Agent → PLC coil을 `ProcessEvent`로 변환 |
