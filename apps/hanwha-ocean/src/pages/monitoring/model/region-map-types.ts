...
function getStatusPalette(status: Region['status']) {
  if (status === 'warning') {
    return {
...
export const resolveMapZones = (regions: Region[]): ResolvedMapZone[] => {
  const regionById = regions.reduce<Record<string, Region>>((acc, region) => {
