...
**가드를 우회한 사례 — philly-terrain.glb (2026-09-08)**: 조선소 주변 지형(필라델피아
시 전역 OSM, 폭 18.9km)은 16bit 그리드가 28.8cm 인데 도로(Y 0.3)·숲(0.1)·지면 평면(0)
층간이 10cm 라 가드가 생략했고, 결과는 f32 123MB 였다. 그 평면층들은 2~57m 기복의
지형 overlay **아래 묻혀 있어** z-fighting 이 보이지 않고 건물·지형은 도시 스케일에서
29cm 양자화가 비가시라, `FORCE_MESHOPT=1` 로 재실행해 14.8MB 로 반입했다.
simplify 는 245만 → 182만에 그쳤다(ratio 0.4 미달) — 건물 벽이 quad 하나짜리 박스라
오차 한도 안에서 더 접을 게 없다. 원본은 175MB 로 GitHub 100MB 한도를 넘어 **커밋하지
않고 컨플루언스에서 별도 관리한다**(`assets-src/README.md` 참고).

```
OK  167.45MB -> 14.84MB (-91.1%)  philly-terrain.glb  [그리드 28.8cm / 층간 10.0cm → meshopt 적용]
```
...
| 광역 지도(수 km 이상)에서 가드가 meshopt 를 생략 | 그리드가 층간 갭보다 커짐 | 갭을 만드는 평면층이 화면에 보이는지 확인. 지형 아래 묻힌 층(philly-terrain 의 도로·숲)이면 `FORCE_MESHOPT=1` 로 우회 |
...
## 9. 교체 배포와 브라우저 캐시 (자동 처리)

최적화한 GLB 를 서버에 올렸는데 **브라우저에 옛 모델이 계속 보이는** 문제는
해결되어 있다. 별도 작업은 필요 없지만, 동작 방식은 알아둘 것.

`public/` 의 GLB·씬 JSON 은 `dist/assets/` 의 번들과 달리 **파일명에 해시가 없다**.
그래서 파일을 교체해도 URL 이 그대로고, 브라우저·앞단 프록시는 "같은 URL = 같은 파일"
로 보고 캐시를 재사용한다. 사용자가 직접 캐시를 비워야 새 모델이 나오던 원인이 이것이다.

지금은 빌드 시 `apps/shell/vite-plugin-asset-hash.ts` 가 `public/` 의 파일 내용을
sha256 해싱해 매니페스트를 만들고, `withBaseUrl()`(`packages/domain/src/3d/lib/asset-url.ts`)
이 모든 로더 URL 에 `?v=<해시>` 를 붙인다:

```
/crane_rnd/maps/phillyshipyard.glb?v=ad00d960
```

- **내용이 바뀐 파일만** 해시가 바뀌므로, 그 파일만 다시 받는다.
  안 바뀐 나머지(대부분의 25MB GLB 들)는 캐시를 그대로 쓴다 —
  배포 전체에 단일 버전을 붙이는 방식과 달리 불필요한 재다운로드가 없다.
- nginx 는 `?v=` 가 **있으면** `immutable`(재검증 왕복조차 없음),
  **없으면** `no-cache`(매번 재검증)로 가른다. `map $arg_v $asset_cache_control` 참조.
- 매니페스트는 별도 JSON fetch 가 아니라 번들에 인라인된다. 매니페스트 파일 자체가
  캐시되면 같은 문제가 한 겹 되풀이되기 때문이다.

따라서 **GLB 를 교체하면 재빌드·재배포만 하면 된다.** 단, `public/` 파일을 바꾸고
빌드를 다시 하지 않으면 해시가 갱신되지 않으니 반드시 빌드를 거칠 것.

### 9-1. HTTP 캐시가 아니었던 재발 사례 — localStorage 씬 고정
...
지금은 localStorage 저장본에 **저장 당시 배포된 씬 JSON 의 콘텐츠 해시**
(`baseVersion`, 매니페스트 해시 재사용)를 봉투로 함께 기록하고, 로드 때
현재 배포 해시와 비교한다:

- 해시가 같으면 → 로컬 편집 유지 (배포가 안 바뀌었으니 안전).
- 다르면(=씬이 재배포됨) → 배포본을 먼저 로드하고, **성공한 뒤에만** 로컬
  저장본을 삭제한다. 배포본 로드가 실패하면(일시 장애·404) stale 로컬로
  폴백한다 — 관제 화면에서 옛 배치가 빈 에러 화면보다 낫다.
- 봉투 이전 포맷(무버전 저장본)은 전부 stale 로 간주한다 — 다음 배포 후
  첫 정상 로드 한 번으로 현장 브라우저의 "영원히 고정된 옛 씬"이 일괄
  정리된다.
- 한계: 이 봉투 포맷을 모르는 **구버전 코드로 롤백**하면 봉투 객체를 씬으로
  읽어 빈 씬이 된다. 롤백 배포를 한다면 이 파일(scene-dev-storage)의 로드
  분기만은 유지할 것.
