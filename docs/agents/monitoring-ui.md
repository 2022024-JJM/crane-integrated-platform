...
- 미니맵 캡처 렌더 타깃은 `stencilBuffer: true`(실루엣 마스크·헐). 캡처 중 `OceanWater` 는 숨기고, 물을 숨겼을 때만 clear color 를 `MINIMAP_SEA_CLEAR_COLOR` 로 바꿨다가 원복한다.
