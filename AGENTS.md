...
- `node scripts/add-model-lod.mjs <파일.glb>` — 정적 모델에 거리별 LOD 체인 추가(이어서 `pnpm optimize:glb`). 상세는 아래 3D 작업 절
...
- 공통 서브라우트: `3d-monitoring`, `3d-viewer-edit`, `virtual-tags`, `detection-settings`, `crane-status`, `work-history`, `alarm-history`, `3d-replay`. `goliath-work` 는 여기에 `vision`, `cabin-monitoring` 을 더 가진다. `3d-monitoring` 은 **실시간(WebSocket 만)**, `3d-replay` 는 **플레이백(분석)** 페이지(메뉴 라벨 "3D 플레이"(`MonitorPlay` 아이콘, 2026-09-18 — 그 전 "3D 플레이백"), 2026-09-16 — 리플레이 | 시뮬레이션 소스를 한 3D 뷰에서 재생하고 우측에 실행 리포트. 아래 기준 파일 표의 "플레이백" 행).
- `3d-viewer-edit` 는 `@crane/widgets` 의 scene editor 를, `virtual-tags` 는 `@crane/widgets/virtual-tags` 의 가상 태그 관리 페이지를 사용하며 세 화면이 공유한다. 가상 태그 목록은 region 무관 전역이다. `detection-settings` 는 `@crane/widgets/detection-settings` 의 감지 설정 페이지(충돌·영역 감지 on/off·정지 옵션·영역 이름 표시, 2026-09-17. 메뉴 라벨은 "설정", `Settings` 아이콘, 작업 그룹 맨 아래)로 같은 패턴이다 — 값은 localStorage `crane:detection-settings` 에 영속되며 region 무관.
...
| 태그 맵핑 편집 UI | `packages/widgets/src/3d/ui/tag-mapping-section.tsx`(인스펙터 탭), `tag-key-combobox.tsx`, `lib/tag-mapping-editor.ts`(충돌 판정·기본값, 테스트 대상) |
| 가상 태그 관리 페이지 | `packages/widgets/src/virtual-tags/ui/virtual-tags-page.tsx` |
| 감지 설정 페이지(메뉴 "설정", 충돌·영역 감지 옵션 5개, 2026-09-17 — 독 팝업·에디터 팔레트 탭에 흩어져 있던 스위치를 한 곳으로) | `packages/widgets/src/detection-settings/ui/detection-settings-page.tsx`(스위치만 — 기록·침범 목록 같은 런타임 상태 없음), 영속화 `packages/features/src/3d/lib/detection-settings-storage.ts`(키 `crane:detection-settings`, 봉투 sanitize, 테스트 대상. 기본값: 감지·이름 표시 ON, 충돌 시 정지·침범 시 정지 OFF(2026-09-18). 두 스토어가 초기값을 `readDetectionSettings` 로 읽고 setter 마다 `writeDetectionSettings`(read→merge→write) 한다) |
...
| 씬 객체 충돌 감지(시뮬레이션·실시간 모니터링과 에디터, 모델↔모델 관통 감지. 골리앗 LiDAR 근접 존인 collision guard 와는 별개) | 기하 `packages/domain/src/3d/lib/collision-volumes.ts`(AABB→OBB→three-mesh-bvh 삼각형, BVH 는 여기서만 접근·빌드 안 함), 런타임 `packages/features/src/3d/model/scene-collision-runtime.ts`(React 밖 싱글턴, 메쉬 matrixWorld 변화 기반 dirty 쌍만 검사, 기준선·억제·시간 예산. hit 을 돌려줄 뿐 스스로 멈추지 않는다), 스토어 `model/use-scene-collision-store.ts`(on/off·충돌 시 정지 여부 — 감지는 기본 ON, 정지는 기본 OFF(2026-09-18), localStorage `crane:detection-settings` 영속(`lib/detection-settings-storage.ts`, 2026-09-17 — 그 전엔 세션 전용이라 새로고침 시 다시 ON) · 기록 최대 10개·활성 기록은 세션 전용. `selectRecord`(기록 복원 = `rigValueStore.restore(스냅샷)` + 러너 정지)는 스토어에 남아 있지만 기록 목록 UI 가 없어져(2026-09-17) 화면에서 부르는 곳이 없다, ▶ 재생이 `resume` 으로 재무장), 훅 `model/use-scene-collision-detector.ts`(**`RigDriver` 바로 다음에 마운트** — 같은 priority useFrame 은 마운트 순. **감지는 `runner` 가 재생 중(+기즈모 드래그 아님)일 때만 돈다**(2026-09-08, `scene-collision-hold.ts` 의 `isRunnerRunning` — 시뮬레이션은 가상 태그 `isRunning`, 실시간은 진입~이탈 내내 재생이라 `held` 여도 스캔). 스캔 재개 전이·기록 복원(`selectRecord`)은 `rebaseline()`, 새 모델 항목(첫 마운트·리마운트·참조 교체)은 런타임 tick 이 스스로 기준선으로 되돌리고, 기준선은 큐가 빈 뒤에도 `BASELINE_SETTLE_MS`(1s) 안정화 창이 지나야 scanning 이 된다 — 정지 중 기즈모·인스펙터·슬라이더로 만든 겹침, 드래그로 놓은 겹침, 로딩 배치, 재생 시작 스무딩은 보고 대신 억제되고 시뮬레이션이 떼었다 다시 붙여야 보고된다. 창 안에서 시뮬레이션이 만든 겹침도 분리 전까지 보고되지 않으며, 재생 중 편집(opacity 만 바꿔도 참조 교체)마다 1초 blind 가 생기는 것은 허용된 부작용이다. 어느 모드든 그 쌍만 억제한 채 감시 계속 — 정지 모드는 러너 pause+freeze+pin(스캔도 멈춤), 무정지는 3초 박스. **실시간 러너는 자동 정지하지 않는다**(2026-09-16 — 실제 장비는 멈추지 않는데 화면만 얼리는 것은 관제에 해롭다. 검출기가 `runner==='realtime'` 이면 pauseOnCollision 과 무관하게 flash). 억제 해제는 모델 AABB 가 아니라 **메쉬 단위**(AABB+margin → OBB → 삼각형 최단 거리 `meshesWithinDistance`)라, 단일 메쉬 크레인처럼 OBB 가 늘 겹치는 모델도 삼각형이 0.05 unit 이상 떨어지면 재보고된다), 설정 UI 는 감지 설정 페이지(`packages/widgets/src/detection-settings/`) 하나다(2026-09-17 — 독 `AlertTriangle` 팝업·에디터 팔레트 "시뮬레이션" 탭·기록 목록·[충돌 지점 보기]를 뺐다. 기록은 스토어에 남아 하이라이트·저널·실행 리포트가 쓴다. 에디터는 러너를 켤 UI 가 없어 충돌 검출기·하이라이트도 마운트하지 않는다)(충돌 시 캔버스 위 오버레이 패널은 2026-09-07 에 제거. 화면 수준 경보는 `ui/scene-collision-alert-overlay.tsx` — 가장자리 inset box-shadow 비네트만(상단 배너·[충돌 지점 보기]·[이어서 재생]은 2026-09-12 에 뺐다 — HUD 충돌 칸·독 배지와 겹쳐 보였다. 재개는 독 ▶ 에 있다. 재개는 러너 ▶ 전이 — `subscribeRunnerResume(runner, cb)` 한 곳이 러너별로 구독한다: 시뮬레이션은 가상 태그 false→true, 플레이백은 리플레이·가상 태그 어느 쪽이든, 실시간은 자동 정지가 없다(아래). 실시간 페이지에는 독 ▶·시뮬레이션 UI 가 없다(2026-09-16)) — 와 씬 안 충돌 장비 일체형 빨간 실루엣 테두리(`@crane/domain/3d` 의 `ObjectSilhouetteOutline` — 스텐실 마스크 + 인플레이션 헐 `lib/silhouette-outline.ts`, `SCENE_GL_OPTIONS.stencil: true` 필요. 에디터의 선택 표시도 같은 컴포넌트를 노랑으로 쓴다 — 모델 전체와 안쪽 노드(계층 목록·drill-in) 모두: `GltfModel` 의 `selectionStyle='outline'`, 스텐실 없는 캔버스에선 기본 'box' 유지. 에디터의 **지도**는 'box' — 수 km 지형을 실루엣으로 두르면 화면 가장자리 전체가 테두리가 되고 헐이 지형 메시를 매 프레임 두 번 더 그린다)·경고 표지 `scene-collision-highlight.tsx`). 모니터링은 **전 모드**(실시간·플레이백·미리보기)에서 켠다(2026-09-16, 리플레이 소스 포함). 값 생산자 정지는 `model/scene-collision-hold.ts` 한 곳 — 러너 종류 `SceneCollisionRunner` 는 `'simulation'`(에디터·미리보기) | `'realtime'` | `'playback'`(활성 소스가 `usePlaybackStore.source` 에 따라 리플레이 러너 또는 가상 태그 러너). 가상 태그·리플레이는 pause(경과·프레임 index 보존), 실시간(WebSocket)은 `useRealtimeStore.hold()` 로 **화면 반영만 보류**(러너가 수신 값을 drain 만 하고 버림, 장비는 계속 움직임)하며 pinned 을 떠나는 모든 스토어 경로(resume·clearActive·clearHistory·clear·setEnabled(false))가 release 한다. 실시간에서 pinned 이 되는 경로는 `selectRecord` 뿐인데 그걸 부르던 기록 목록 UI 는 2026-09-17 에 뺐다. 상수 `lib/scene-collision-pairs.ts` |
| 모델 영역(zone) 침범 감지(2026-09-12 도입. 설정은 감지 설정 페이지 — 충돌 감지와 **별도 스토어**, 정지·자세 복원 없음) | 스키마 `SavedModelInfo.zones?: SavedModelZone[]`(`packages/domain/src/3d/model/types.ts` — id·name·`#rrggbb` color·radius(씬 unit, position 과 같은 단위 — 옥포는 11.7 m/unit 이라 "m" 이 아님)·`offset?: [dx,dz]`(월드 축, [0,0] 생략). 비면 필드 생략), 방어 `lib/sanitize-model-zones.ts`(반경 ≤0·NaN 항목 버림, 중복 id first-wins, 색은 `#rrggbb` 만 소문자 정규화·그 외 `DEFAULT_ZONE_COLOR` 폴백 — 저장소 첫 색 sanitize. `sanitize-scene-info` 가 모델마다 부른다), 기하 `lib/zone-volumes.ts`(판정은 **Y 무시 무한 수직 원기둥**. `meshIntersectsVerticalCylinder` 는 원기둥을 메쉬 로컬로 보내지 않고 BVH 노드 박스·삼각형을 `matrixWorld` X·Z 행으로 월드 XZ 에 투영해 2D 원↔삼각형 판정 — 기울어진 메쉬·비균일 스케일도 정확. BVH 없으면 `null`, AABB/OBB 로 대신 답하지 않는다(회전한 긴 붐의 박스는 오탐이고 진입 기록은 영구 행)). 런타임 `packages/features/src/3d/model/scene-zone-runtime.ts`(React 밖 싱글턴 `sceneZoneRuntime`, 충돌 런타임과 같은 골격 — registry 루트, 메쉬 `matrixWorld` 서명 dirty, job 사이 시간 예산(2ms). **상태 기반**: 기준선·억제 없음, 안에 있으면 침범 중이고 전이(enter/exit)만 돌려준다. 영역 중심은 소유 루트 `matrixWorld` 평행이동+오프셋을 `rootLast` 서명으로 따로 추적(트롤리만 움직이면 침범자로만 재검사). 영역×다른 모델 메쉬는 AABB XZ → 삼각형, 영역×다른 모델 영역은 중심 거리 ≤ r1+r2 인라인(같은 모델 영역끼리·소유 모델 자신은 제외, 전이는 a 쪽 한 번). 히스테리시스 진입 r·이탈 r+`zoneExitMargin(r)`(=max(0.05, r×3%), `lib/scene-zones.ts` — 떨림 원인이 PLC 주행값 노이즈라 반경 비례). inside 집합은 zoneKey(`modelId#zoneId`)로 런타임이 들고 있어 항목 재생성(반경·이름 편집·리마운트)에도 유지되며, 합성 exit 의 기준은 job 존재가 아니라 **씬 모델 목록**(sync 직후 재해석 전 쌍을 사라진 것으로 보면 이름만 바꿔도 exit+enter). 씬에 영역이 없으면 변화 감지도 건너뛰어 비용 0), 스토어 `model/use-scene-zone-store.ts`(`enabled`·`labelsVisible` 은 기본 ON, `stopOnIntrusion` 은 기본 OFF(2026-09-18)·셋 다 localStorage `crane:detection-settings` 영속(2026-09-17), `intrusions`(침범자 ≥1 영역만, 영역↔영역은 양쪽 항목에 거울상). **기록(history)은 없다** — 상태만(2026-09-12 사용자 요청으로 제거)), 검출기 `model/use-scene-zone-detector.ts` + `ui/scene-zone-detector.tsx`(`RigDriver` 뒤·`SceneCollisionHighlight` 다음, 50ms 스로틀. **스캔은 러너 여부와 무관**(에디터 드래그 피드백) — 러너 prop 없음. `moved`·전이 프레임에 `invalidate()` 로 demand 캔버스의 마지막 자세 미검사·링 낡음 방지), 링 `ui/scene-zone-rings.tsx`(`SceneZoneRings`, 검출기 **바로 다음**. 모델 루트에 portal 하지 않고 씬 수준 형제로 useFrame 마다 registry 루트 월드 위치를 따라감(스케일·yaw 상속으로 반경 단위가 깨지는 것 방지), 단위 지오메트리+그룹 scale=r, `meshBasicMaterial toneMapped=false`(solar 밤에도 색 식별), renderOrder 1·2(바다 0.25 위), key 는 zoneKey(중심값 key 는 매 틱 리마운트). 침범은 자기 색의 채움↑·2Hz 펄스 — **색은 식별자일 뿐 상태를 실어 나르지 않는다**(빨강 덮어쓰기 없음). 이름 배지 `Html zIndexRange [4,0]`, 침범 중엔 배지 우상단에 빨간 점·흰 숫자(그 영역의 침범자 수 — 모델 라벨이 아니라 **영역 배지**에 붙는다). 배지는 스토어 `labelsVisible`(감지 설정 페이지 "영역 이름 표시", 영속·기본 ON)이 꺼지면 통째로 언마운트된다 — 감지·링은 그대로. 에디터는 `selectedModelId` 를 넘겨 **토글 off 여도 선택 모델의 영역은 그린다**(반경 편집 피드백, 판정은 멈춤). 거버너 활성 소스에 `intrusions.length>0` 추가), 설정 UI 는 감지 설정 페이지(독 `Radius` 팝업·에디터 팔레트 "영역" 탭·침범 목록 패널은 2026-09-17 에 뺐다 — 현재 침범은 알람 목록·HUD 침범 칸·링·비네트가 보여 준다), 미니맵 `scene-minimap.tsx` `drawZones`(마커 아래, 같은 중심·색, 침범 시 채움 진해짐, 2px 미만 생략). 모니터링은 **전 모드**에 마운트, 에디터는 `scene-objects-edit-canvas.tsx`. 검출기는 `runner` prop(기본 `'simulation'`)으로 ▶ 재개 전이만 본다(스캔 게이트는 아니다). 인스펙터 "영역" 탭 `packages/widgets/src/3d/ui/zone-section.tsx`(+이름·색(`<input type="color">`)·반경·오프셋 X/Z 카드, 수치 입력은 트랜스폼 위치처럼 " m" 표기. 순수 로직 `lib/zone-editor.ts` — 새 영역 반경은 `measureModelFootprintRadius`(루트 원점에서 보이는 메쉬 월드 AABB 합집합의 가장 먼 XZ 모서리까지 거리 = 모델을 딱 감싸는 원, 0.1 반올림, 미로드면 10. 가로 폭 전체를 반경으로 잡았던 첫 버전은 링 지름이 모델의 두 배였다)·추가 시 이름은 "영역 n" 을 실제 값으로 저장(영역 패널·배지에 id 노출 방지)·6색 프리셋 순환), 편집 채널 `updateSelectedZones`(`use-selected-scene-object-editor.ts`, 세션·페이지에 배선), **dirty·히스토리는 `scene-snapshot.ts` 의 `isZoneListEqual` 이 잡는다**(빠지면 편집이 동등 단락에 먹혀 저장 안 됨). i18n `monitoring:inspector.zones.*`·`monitoring:detectionSettings.*`·`monitoring:sceneZone.{unnamed,viewZone}`. **제외 토글**: `SavedModelInfo.zoneExempt`(true-only, 영역 탭 체크박스 "영역 감지에서 제외" — 켜면 아래 영역 목록이 비활성·흐림 처리되고 값은 보존) — 그 모델이 감지에서 통째로 빠진다: 침범자도 되지 않고 **자기 `zones` 도 감지하지 않는다**(`buildEntry` 가 ZoneEntry 를 만들지 않는다). 링도 그려지지 않는다(`SceneZoneRings` 가 같은 플래그를 보고 선택 중이어도 건너뛴다) — 정의는 남아 제외를 풀면 되살아난다. 침범 중 제외로 바뀌면 양쪽 다 합성 exit(`liveZoneKeys`·`liveIntruderIds` 가 같은 기준). **등급·정지(2026-09-12 추가)**: `SavedModelZone.level?: 'warn'|'stop'`('stop' 만 저장, sanitize·`isZoneListEqual` 포함, 인스펙터 영역 카드의 경고/정지 토글). 'stop' 영역 진입은 스토어 `stopOnIntrusion`(영속·기본 OFF, 감지 설정 페이지 스위치)일 때 충돌과 같은 `holdRunners`(시뮬레이션 pause·실시간 화면 반영 보류)로 멈추고 `held` 에 그 쌍 하나를 둔다. **실시간 모니터링은 멈추지 않는다**(2026-09-16 — 검출기가 `applyTransitions(transitions, allowHold=false)` 로 넘긴다). 재개는 러너 ▶ 전이(`subscribeRunnerResume` — 시뮬레이션·플레이백. 영역 패널의 재개 버튼은 패널과 함께 2026-09-17 에 뺐다) — `resume` 은 `releaseRunners` + 그 쌍을 `acknowledged` 에 넣어 **이탈 전까지** 다시 멈추지 않는다(아니면 재생 즉시 같은 침범으로 또 멈춘다). 이탈·감지 off·clear·stopOnIntrusion off 는 정지·승인을 푼다. 화면 경보 `ui/scene-zone-alert-overlay.tsx`(비네트 amber/red 만 — 배너는 2026-09-12 에 뺐다, 충돌 경보가 떠 있으면 아무것도 안 그림). [영역 보기]는 우상단 알람 목록의 `zone_intrusion` 행 버튼(features/alarm `AlarmFullscreenOverlay onViewZone` ← 페이지가 `Monitoring3dView actionsRef.viewZone` 으로 잇는다 — 두 슬라이스는 같은 레이어라 서로 import 하지 않는다. `computeCollisionViewPose` 재사용. 행의 색 점은 알람 `eventData.color`, 파싱은 `@crane/domain/alarm` `getZoneAlarmMeta`. 이 목록의 배지는 심각도 분류명(위험/높음/중간)이 아니라 위험 수준 `getAlarmRiskLevelLabel`(위험/경고/주의/정보)이고 설명 줄 대신 발생 시각을 보인다(2026-09-17). 골리앗 실시간 화면은 알람 오버레이 자체가 없어 아직 없다, 2026-09-17), 재개는 독 ▶. HUD 에 침범 칸(씬에 영역이 있을 때만, 정지 중엔 "n · 정지"). 가동 칸은 값 생산이 멈춘 동안(충돌 pinned·영역 hold·실시간 보류·시뮬 정지) "정지 중". 미니맵: 마커 클릭 = 그 모델 포커스(포커스 중 재클릭 = 돌아가기, 드래그 시작 안 함·커서 pointer), 영역 원 hover = 소유·이름·침범자 목록 라벨. v1 에서 뺀 것: 높이 제한·무시 목록·저널 영속화·m 단위 표시·yaw 기준 오프셋(`zoneCenterWorld` 한 곳) |
...
| 장비 운전 상태(2026-09-12 도입) | `@crane/core/types/status` 의 `EquipmentRuntimeStatus`(running·idle·offline·unknown). PLC 상태 태그가 아니라 **태그 값 버스 활동**에서 파생 — `tag-value-bus.ts` 의 `TagLiveValue.changedAt`(값이 달라진 마지막 시각)과 `at` 을 모델 `tagMappings` 의 tagKey 들로 모아 판정(`packages/features/src/3d/lib/model-runtime-status.ts`, 창 running 8s(리플레이 5s 프레임보다 넓게)·offline 20s, 테스트 대상). craneId 없는 필리 모델도 맵핑만 있으면 상태가 나온다. 훅 `model/use-model-runtime-statuses.ts`(1Hz 폴링, 같으면 참조 유지)를 `Monitoring3dView` 가 한 번 부르고 라벨(`@crane/domain/3d` `ModelLabel` 의 `runtimeStatus` prop — 알람이 없으면 배경을 상태색으로 물들이고 이름 앞 점, offline 은 라벨 흐림. 알람이 있으면 배경은 알람색·점만 남음)·미니맵 마커 색(알람 > 상태 > 기본)·관제 HUD 가 공유한다. 색 팔레트는 `RUNTIME_STATUS_COLORS`(hex) 와 model-label 의 Tailwind 클래스 두 곳 — 함께 바꾼다 |
| 플레이백 페이지(리플레이 + 시뮬레이션, 실행 리포트, 2026-09-16 도입) | 뷰 `packages/features/src/3d/ui/playback-view.tsx`(`PlaybackView` — `Monitoring3dView mode='playback' toolbarLayout='dock'` 위에 소스 탭 `ui/playback-source-tabs.tsx`(리플레이|시뮬레이션 — 탭 라벨은 2026-09-18 에 "기록 리플레이" 에서 줄임, role=tablist 버튼 — 공용 Tabs 없음)과 트랜스포트 바 `ui/playback-transport-bar.tsx` 를 **상단**에 둔다(2026-09-17 에 하단에서 올림). 소스 `model/use-playback-store.ts`(`usePlaybackStore.source` 'replay'|'simulation', 세션) — 전환은 **리마운트가 아니라 상태 전환**(`key={source}` 는 useSceneData 진입의 resetReplay 로 프레임을 잃고 loadFrames effect 가 다시 돌지 않는다) 이라 씬·GLB·카메라·검색 상태·프레임이 유지되고 PlaybackView effect 가 떠나는 소스만 정리(시뮬레이션 → `stopSimulation`, 리플레이 → pause+seekTo(0)). `useSceneData(mode='playback')` 는 진입에 resetReplay + 가상 태그 로드만(자동 재생 없음). 트랜스포트 어댑터 `model/playback-transport.ts`(`PlaybackTransport` — isPlaying·durationMs(null=열린 구간)·speed·play/pause/seek(ms)/stepFrames, `readPlaybackTransport()` 스냅샷·`usePlaybackTransport()` 훅(위치는 포함하지 않는다 — `readPlaybackPositionMs()` 를 폴링). 리플레이는 프레임 누적 ms ↔ index 를 `lib/replay-position.ts` 로, seek 는 충돌 `clearActive`(옛 자세를 가리키는 pinned 박스 방지). 바·마커 seek·통계·`scene-collision-hold` 가 전부 이것만 본다). 바: 소스 슬롯(리플레이=구간 검색 팝오버 `ReplaySearchForm`, 시뮬레이션=`SceneSimulationPanel` 팝오버) · 이동/▶ · 사건 마커 띠가 얹힌 스크럽 · 배속(소스별 선택지) · 위치. 캔버스 **위 별도 행**(오버레이 아님)이라 HUD·좌상단 열과 겹치지 않는다(옛 상단 `ReplayPlayerControls`·`Replay3dView` 는 삭제). 시간 축은 씬 시간(리플레이 프레임 누적 ms·시뮬레이션 러너 경과 ms). **실행 통계**: 순수 집계 `lib/playback-stats.ts`(`computePlaybackStats` — 사건은 `atMs ≤ windowEndMs`(현재 위치) 로 창에 걸러 뒤로 seek 하면 감춰질 뿐 지워지지 않고, 장비 상태·태그 누적은 단조 누적, 정지(hold) 시간만 벽시계(씬 시간은 정지 중 흐르지 않는다). 영역 체류는 진입~이탈, 미이탈은 창 끝까지, 진입 없는 이탈 무시. 회차 = floor(경과/시나리오 길이). 태그 `saturatedMs` = 연속 publish 속도가 `limits.maxSpeed×0.98` 이상인 씬 시간(시뮬레이션만 — 러너가 한계로 자르므로 '최고 속도' 는 정보가 없다), 테스트 대상)·표시 보조 `lib/playback-format.ts`. 스토어 `model/use-playback-stats-store.ts`(세션 전용·영속화 없음. `data` 는 기록기가 **제자리 갱신**하고 `version` 을 4Hz 이하로 bump — publish 마다 setState 금지. `reset(meta)` 가 실행의 시작점: 마운트·소스 전환·새 구간 조회(프레임 참조 교체)·시나리오 변경·시뮬레이션 종료(hasSession true→false). resetValues·seek(0) 은 뒤로 seek 일 뿐 reset 아님). 기록기 `model/use-playback-stats-recorder.ts`(PlaybackView 가 Canvas 밖에서 한 번 마운트: 충돌 기록·영역 diff·정지(충돌 pinned ∪ 영역 held) 전이·두절 전이를 사건으로 — 사건 시각은 스토어의 벽시계 `at` 이 아니라 구독 콜백 안에서 읽은 트랜스포트 위치(같은 틱, 동기). 250ms 폴링으로 재생 중 지나간 구간을 `scanned`(합집합)·장비 상태 ms 에 더하고(한 폴링에 배속×주기×6+500ms 넘게 뛰면 seek 로 보고 제외), 충돌 감지 off 를 `detectionOffSeen` 으로. 태그는 버스 관찰자 `subscribeTagValues`(`tag-value-bus.ts`, 소비자 슬롯과 별개·다중)로 publish 마다 집계(맵핑된 키만). 운전 상태는 `useModelRuntimeStatuses(scene, { paused, timeScale })` — 정지 중 재판정을 건너뛰고 창을 1/배속(`scaleStatusWindows`)으로 조정해 일시정지 뒤 전 장비 두절·0.5배속 idle 깜빡임을 막는다(`Monitoring3dView` 도 플레이백에서 같은 옵션). 리포트 `ui/playback-report-panel.tsx`(조립. 2026-09-16 재설계 — 레퍼런스: Foxglove State Transitions(스윔레인)·ISA-18.2 bad actors(원인 상위)·MoTeC 채널 리포트(range bar). PASS/FAIL 판정은 두지 않는다(사용자 결정). 순서: 헤더(실행·창·검사된 구간·회차 — CSV 내려받기는 2026-09-17 에 뺐다) → KPI 카드 `ui/playback-report-kpi.tsx`(큰 숫자 + 씬 시간 축 누적 스텝 스파크라인 SVG(`sparklinePath`), 현재 위치 세로선. 충돌·침범·정지는 `cumulativeSeries`, 가동 비율은 `runningRatioSeries`) → 스윔레인 타임라인 `ui/playback-report-timeline.tsx`(행 = 사건(충돌 ◆·정지 밴드) + 장비별 상태 밴드(`statusBands`, 색 `PLAYBACK_STATUS_FILL` = `RUNTIME_STATUS_COLORS`) + 영역 체류 밴드(`zoneBands`, stop 은 red·미이탈은 점선 끝). 검사 안 된 구간은 빗금, 현재 위치 세로선, 축 클릭 = `msAtFraction` 으로 seek. HTML 절대 배치 % 라 SVG 늘림 없음) → 원인 상위 `RankingBars`(충돌 쌍·영역×침범자 상위 5, `rankZoneIntruders`·`pairRankingRows`/`zoneRankingRows`) → 장비 적층 비율 막대 → 영역 체류 막대 → 태그 range bar(`tagRangeBar` — 시뮬레이션은 가상 태그 정의 min~max 기준, 리플레이는 관측 범위) + 이동량 + 포화 막대 → 사건 목록(종류 필터 칩 `PLAYBACK_EVENT_FILTERS`, 클릭 = seek, 리플레이는 한 프레임·시뮬레이션은 0.5s 앞 — `markerSeekLeadMs`). 표·목록은 `ui/playback-report-tables.tsx`(컴포넌트만 — 행 변환·상수는 `lib/playback-format.ts`, react-refresh 규칙). 밴드의 출처는 기록기가 남기는 `statusTransitions`(모든 전이, reset 직후 현재 상태를 unknown→x 로 심는다). 차트는 전부 인라인 SVG/DIV — features 에 recharts 를 넣지 않는다. 세 앱 `replay-monitoring-view.tsx` 가 `ResizablePanelGroup` 우측(34%)에 둔다). HUD 연결 칸은 `playbackPlaying/playbackPaused`, 리플레이 소스는 `timeSource='replay'`(현장 시각·조명이 프레임 시각)·`SceneClockMenu` 숨김. i18n `monitoring:playback.*`. **v2 로 미룬 것**: 두 실행 비교, 리플레이 구간 서버 알람, 서버 timeline 가동 구간, 태그 시계열 차트, 검사 구간의 기준선·감지 off 정밀 차감, 프레임 전체 헤드리스 스캔, PDF·대시보드 위젯. **전제**: 감지·리포트는 씬에 `tagMappings` 가 있어야 의미가 있다 — 지금은 philly-2dock(가상 태그만)·dock-in(서버 리플레이만)뿐이고 옥포 실외·골리앗 씬은 맵핑이 없어 아무것도 움직이지 않는다 |
| 관제 요약 HUD(상단 중앙, 2026-09-12 도입) | `packages/features/src/3d/ui/scene-status-hud.tsx` — 현장 시각(`useSceneSunState`, 리플레이는 프레임 시각)·풍속/풍향(`model/use-scene-weather.ts`: `scene-site-geo` 좌표로 open-meteo 10분 주기 — 헤더 `useHeaderWeather` 는 라우트·옥포 독 좌표뿐이라 따로 둠. `@crane/domain/weather` 의 `WeatherSnapshot` 에 `windSpeed`(m/s)·`windDirection` 추가, 응답에 없으면 null)·가동 n/상태 확인 N·두절(0 이면 숨김)·알람 장비 수(페이지가 넘긴 `alarmsByCraneId`, 최고 severity 색). 풍속 권고 단계는 `lib/wind-advisory.ts`(주의 10·중지 15 m/s, 현장 규정에 맞춰 상수만 조정). 칸 추가(2026-09-12): 영역 침범 수·충돌 상태(활성일 때만)·연결(`model/use-realtime-connection-state.ts` — 시뮬레이션 재생/정지, 실시간은 cranes-lite WebSocket `subscribeState` 상태에 화면 반영 보류를 덧입힘, 리플레이. `mode`·`sceneInfo` prop). pointer-events-none. 독 배치의 실시간 화면에서만 마운트한다 — 플레이백은 미니맵과 함께 뺐다(2026-09-16)(상단 중앙을 같이 쓰던 충돌·영역 배너는 2026-09-12 에 제거) |
| 경보 알림 채널(2026-09-12 도입) | `packages/core/src/lib/alert-notifications.ts` — `notifyAlert({id, severity, title, description, toast?})` 가 toast(sonner, 앱 전역 Toaster)·소리(WebAudio 두 음/세 음 비프, 에셋 없음·사용자 입력 전엔 무음)·브라우저 알림(탭이 숨겨진 때만, 권한 granted 일 때만)을 설정(`useAlertNotificationSettings`, localStorage `crane:alert-notify`, 기본 소리 ON·브라우저 OFF — 페이지 설정 팝업 "경보 알림" 절)에 따라 내보낸다. i18n 을 모르므로 제목·본문은 호출자가 번역. 발신자: `features/3d/ui/scene-alert-notifier.tsx`(앱 셸 runtime effects — 충돌 새 기록·영역 새 침범 쌍. **toast 없음** — 배너·HUD·헤더 배지와 겹쳐 보여 뺐다, 소리·브라우저 알림만)와 `features/alarm/model/use-critical-alarm-banner.ts`(critical·high 새 알람 — 배너가 있으니 toast 없이 소리·브라우저만). 영역 침범 로컬 알람은 배너 대상에서 제외(`eventType 'zone_intrusion'`) — 3D 화면 경보와 겹침 방지 |
| 영역 침범 → 알람 계통(2026-09-12) | 로컬 알람 `@crane/domain/alarm` `createLocalAlarm`/`localAlarmActiveKey`(id `local:<type>:<subject>:<at>`(+`:clear`), `eventType 'zone_intrusion'`), 스토어 `useRealtimeAlarmStore.upsertLocalAlarm(alarm, key)`. 브릿지는 **앱 셸** `apps/shell/src/runtime/zone-alarm-bridge.tsx`(features/3d ↔ features/alarm 은 같은 레이어라 서로 import 하지 않는다) — 침범 쌍 생성=발생(stop → high, warn → medium), 소멸=해제. `eventData` 는 `zoneKey`·`intruder`·`color`(영역 색 — 알람 목록의 색 점·[영역 보기]가 `getZoneAlarmMeta` 로 읽는다). 실시간 화면의 침범만(`filterAcceptedZoneTransitions` — 위 저널 행과 같은 규칙, `openedAt` 이 승인 집합). craneId 는 소유 모델 craneId, 없으면 모델 id 라 지역 필터는 `isAlarmInRegion`(레지스트리 craneId **또는** `alarm.regionId === regionId`)으로 통과 — 알람 슬라이스의 6개 필터 지점이 전부 이 헬퍼를 쓴다 |
| 영역·운전 상태 저널(2026-09-12) | `@crane/domain/journal` 의 `ZoneJournalEntry`(진입·이탈 각 항목, 이탈에 `durationMs`)·`StatusJournalEntry`(**통신두절 진입·복귀만** — 가동↔대기는 제외, 첫 unknown→x 도 제외), 키 `crane:zone-journal`·`crane:status-journal`. features/3d: 순수 diff `lib/zone-journal-map.ts`(`diffZoneIntrusions` — 두 스냅샷의 쌍 차이로 사건 복원, 알림·알람 브릿지도 같은 함수)·`lib/status-journal-map.ts`, 스토어 `model/use-{zone,status}-journal-store.ts`, 동기화 `ui/zone-journal-sync.tsx`(runtime effects)·`model/use-status-journal-sync.ts`(`Monitoring3dView`, `enabled` = 실시간만). 대시보드 소비는 아직 없음. **실시간 화면의 사건만 남긴다**(2026-09-16): `useSceneInfoStore.activeMode`(useSceneData 가 진입 set·이탈 null)와 `isRealtimeSceneActive()` 로 충돌 기록·영역 진입·두절 진입을 받고, 영역 이탈은 `filterAcceptedZoneTransitions`(`lib/zone-journal-map.ts`)로 **진입을 승인했던 쌍만** 받는다 — 언마운트 cleanup 은 부모(useSceneData)가 자식(검출기 clear)보다 먼저라 activeMode 만 보면 실시간을 떠날 때의 이탈이 버려진다. 플레이백·에디터·미리보기의 사건은 저널·로컬 알람·경보 소리 어디에도 가지 않는다(플레이백은 실행 리포트가 대신) |
| 씬 unit 스케일 | `packages/domain/src/3d/model/scene-unit-scale.ts` `getSceneMetersPerUnit(regionId)` — 옥포 3개 region 11.7, 필리 2개 1(미등록 1). 인스펙터 영역 탭의 반경·오프셋만 m 로 환산해 보여 준다(저장은 unit, `widgets/3d/lib/zone-editor.ts` `unitsToDisplayMeters`/`displayMetersToUnits`). 트랜스폼 위치 필드는 아직 unit 을 " m" 로 보인다(다음 단계). 골리앗 LiDAR `METERS_PER_UNIT 1.17` 은 센서 보정값이라 별개 |
| 모니터링 2D 미니맵(좌하단, 2026-09-12 도입) | 순수 계산 `packages/features/src/3d/lib/minimap.ts`(bounds→프레임, 픽셀↔월드, 팬 포즈, 카메라 발자국 — 테스트 대상)·픽셀 후처리 `lib/minimap-image.ts`(렌더 타깃 readback 은 톤매핑·sRGB 가 안 걸려 있어 ACES→sRGB 를 JS 로 적용, 밤 스냅샷은 평균 휘도 기반 자동 노출 ×1~6), 스토어 `model/use-scene-minimap-store.ts`(스냅샷·표시 여부 영속 `crane:scene-minimap:visible`·새로 고침 카운터, fov/종횡비는 mutable `minimapCameraInfo`), 캡처 `ui/scene-minimap-capture.tsx`(Canvas 안. 씬 준비 + 로더 idle 뒤 useFrame 에서 기준 지도(`resolveCameraBoundsMaps` 합집합 — 카메라 이동 제한과 같은 기준이라 옥포처럼 지도 GLB 에 넓은 외곽 지면이 있으면 그만큼 넓게 나온다) 위를 직교 카메라(up=(0,0,-1) → 이미지 위 = −Z)로 렌더 타깃(**stencilBuffer 필수** — 없으면 바다가 야드를 덮는다)에 한 번 그려 readback. 지도 목록 변경·수동 새로 고침에만 재캡처), 표시 `ui/scene-minimap.tsx`(DOM 2D 캔버스, 66ms 인터벌로 레지스트리 월드 위치 마커(알람 severity 색·포커스 테두리, 루트→AABB 중심 오프셋은 첫 관측 때 캐시)와 카메라 부채꼴·타깃 십자를 직접 그림 — setState 없음. 누르기·끌기는 `panPoseToPoint` 로 타깃만 옮기는 팬을 컨트롤러 `moveTo` 로 보내 `SceneCameraLimits` 가 그대로 걸린다), 독 토글 `ui/scene-minimap-toggle.tsx`. 패널은 상단 그립 바를 끌어 캔버스 영역 안 어디든 놓을 수 있고 위치는 `crane:scene-minimap:position` 에 영속·복원 시 `clampPanelPosition` 으로 창 안에 넣는다(null = 기본 좌하단). `Monitoring3dView` 의 `toolbarLayout='dock'` 이고 `mode!=='playback'` 일 때만 마운트(실시간 관제 화면 전용 — 플레이백은 2026-09-16 에 HUD 와 함께 뺐다, 작은 뷰 없음) |
| 모니터링 씬 독(dock: hover 펼침·고정 우측 레일) | 껍데기 `packages/ui/src/organisms/scene-dock.tsx`(`SceneDockRail`·`SceneDockRailSeparator`, 완전 제어형, 도킹 프레임은 `three-scene-viewer.tsx` 의 `toolbarPlacement="dock"`. 레일 순서는 위에서부터 카메라(원래위치·탑뷰·**저장한 뷰**(`toolbarTrailing`)·확대·축소·전체화면) → 화면 표시(페이지가 준 `toolbarExtras` 알람 토글·골리앗 가드·미니맵·현장 시각). 씬 감지 묶음(시뮬레이션 ▶·충돌·영역)과 그 아래 구분선은 2026-09-17 에 뺐다(감지 설정 페이지로). `toolbarTrailing` 을 카메라 묶음 안에 끼우는 것은 dock 배치뿐이고 가로 툴바는 기존대로 앞에 붙는다, 2026-09-12), 상태·영속화 `packages/features/src/3d/model/use-scene-dock.ts` + `lib/{dock-hover-state,dock-storage}.ts`(순수 리듀서·pin 영속화, 테스트 대상). 조립은 `Monitoring3dView` 의 `toolbarLayout="dock"`. 하단 독 패널(크레인 실시간 상태 테이블)은 2026-09-03 에 제거됐다 |
| 씬 로딩 뒤 후처리(워밍업) 상태 표시 | 전역 워밍업 큐 `packages/domain/src/3d/lib/bvh-build-queue.ts`(`ModelMesh` 가 넣고 프레임급 간격의 고정 예산(8ms) 슬라이스로 처리, 작업 종류 `bvh`(클릭 raycast BVH) → `outline`(실루엣 테두리용 스무딩 노멀 사본, `enqueue` 옵션 `outline: true` 인 메시만 — `GltfModel`/`ModelMesh` 의 `prepareOutline`, 에디터·모니터링의 **모델**만 켜고 지도는 제외. 2026-09-08 추가 — 첫 선택·첫 충돌에서 사본 생성(골리앗+LLC 약 180ms)이 프레임을 세웠다) 순, 각 종류 안에서 삼각형 수 오름차순이라 큰 지형이 마지막, `(지오메트리, 종류)` 참조 카운트라 `cancel` 은 `enqueue` 와 같은 옵션으로, 테스트 대상. 인스턴스별 유휴 콜백 체인은 2026-09-08 에 이 큐로 통합 — 유휴 시간에만 맡기면 로딩 직후엔 콜백이 타임아웃으로만 돌아 지오메트리 90개 씬이 수십 초 걸렸다), 셰이더 프리워밍 `packages/domain/src/3d/ui/silhouette-outline-warmup.tsx`(Canvas 안, 헐·마스크 프로그램을 동기 `compile` 로 미리 컴파일하고 — `compileAsync` 는 폴링 중 캔버스 언마운트·머티리얼 dispose 에 three 내부가 던져 2026-09-12 에 교체 — 헐 머티리얼을 캔버스 수명 동안 들고 있는다 — three 는 같은 셰이더의 마지막 머티리얼이 dispose 되면 프로그램을 지워, 이게 없으면 재선택마다 재컴파일된다. `SceneCollisionHighlight` 옆에 마운트), 단계 선택 `packages/features/src/3d/lib/scene-warmup-step.ts`(bvh 잔여 → outline 잔여 → 충돌 런타임 `baseline`(스토어 `baselinePending`, 검사기가 phase 변화 시에만 갱신하며 **스캔 중일 때만** true — 러너 정지·드래그 중엔 내린다. 재생 시작·드래그 종료·편집 뒤 안정화 창 동안 잠깐 보인다) → drei `useProgress` 활성 순, 타이머 추측 없음), 훅 `model/use-scene-warmup-step.ts`, 표시 `ui/scene-warmup-indicator.tsx`(스피너+문구 한 줄, 비차단, `common:viewer3d.warmup.*`). 모니터링·리플레이는 overlay 슬롯의 좌측 상단 열(포커스 복귀 버튼과 세로 스택), 편집은 캔버스 컨테이너 좌측 상단에 둔다. 초기 로딩 오버레이(`scene-loading-overlay.tsx`)는 그대로이며 이 표시는 오버레이가 걷힌 뒤 이어진다 |
...
- **모니터링·리플레이 캔버스는 `frameloop='demand'` 이고 프레임은 `SceneFrameGovernor`(`packages/features/src/3d/ui/scene-frame-governor.tsx`, 판정 `lib/frame-governor.ts`)가 만든다** (2026-09-11 유휴 발열 절감). 애니메이션 소스(가상 태그 재생, 실시간은 최근 1.5초 안에 메시지를 받은 경우만(`useRealtimeStore.activity.lastMessageAt`), 리플레이 재생, 기즈모 드래그, 바다 EXR 씬)가 있으면 30fps 틱, 멈춘 뒤 1.5초 유예(스무딩 정착), solar 태양만 있으면 5초에 한 번, 정지 씬은 틱 없음 — 조작은 OrbitControls·표면 카메라가 스스로 invalidate 해 주사율로 그려진다. **씬을 매 프레임 바꾸는 새 경로를 추가하면 거버너의 소스 목록에 넣거나 스스로 `invalidate()` 해야 한다** — 안 하면 화면이 다음 조작까지 낡은 프레임에 머문다(model-mesh 의 머티리얼 effect 가 선례). **에디터 캔버스도 2026-09-12 부터 'demand' 다**(`scene-objects-edit-canvas.tsx`, 같은 `SceneFrameGovernor` 마운트) — 정지 화면은 0fps, 조작·React 커밋·재생 중에만 그린다. React 밖에서 씬을 바꾸는 코드는 Canvas 를 모르므로 **프레임 요청 깔때기 `requestSceneFrame()`**(`packages/features/src/3d/model/scene-frame-request.ts`, `SceneFrameGovernor` 가 자기 `invalidate` 를 등록)을 부른다 — 값 저장소 `rigValueStore` 의 set/reset/restore(슬라이더·태그 ingest·기록 복원), `SceneLighting` 의 조명 설정·씬 시계 구독, `SceneEnvironment` 의 배경 교체가 이미 배선돼 있고, 스무딩 잔여는 `useRigDriver` 가 `hasPendingSmoothing()` 으로 프레임마다 다음 프레임을 스스로 요청해 러너 없이 들어온 값도 끝까지 수렴한다 — 단 거버너가 주기 틱을 돌리는 동안(`isSceneFrameTickerActive()`, `scene-frame-request.ts`)은 부르지 않는다. useFrame 안에서 부른 invalidate 는 R3F 가 `frames=2` 로 두어 rAF 루프가 주사율로 자체 지속되므로, 이게 없으면 재생 중 30fps 상한이 무력화된다(2026-09-16 발견). drei OrbitControls·TransformControls·Text 와 R3F 리컨실러(prop 변경·마운트)는 스스로 invalidate 한다. 에디터 perf HUD 의 드로우콜·삼각형은 GizmoHelper 의 Hud 가 마지막으로 그린 기즈모 씬 값(9 calls)이라 참고하지 않는다 — fps·ms 만 본다. 값 저장소의 스무딩 그림자 무효화는 임계 2cm·0.02° 를 넘긴 **프레임마다**(`rig-value-store.ts` `SHADOW_STEP_EPS`) — 벽시계 50ms 스로틀(20Hz 상한)은 2026-09-16 에 뺐다: 렌더가 주사율로 도는 동안 3프레임 중 2프레임이 이전 자세의 depth map 이라 배속 ≥2 에서 자기 그림자 오차가 명멸했다. shadow pass 상한은 거버너 fps 가 정한다. **절감은 핵심(야드 지도 kind 'ground'·크레인)이 아니라 주변에서 한다** — DPR 상한은 1.5 그대로(`SCENE_DEFAULT_DPR` + `three-scene-viewer.tsx` 기본값, 함께 바꿀 것. 1.25 로 내렸다 되돌림), 컨텍스트 지형(kind 'context')은 `GltfModel shading='lambert'` 로 PBR 대신 Lambert(`packages/domain/src/3d/lib/lambert-material.ts`, 원본 머티리얼당 변환본 캐시 — 조명·낮/밤엔 똑같이 반응하고 스펙큘러·radiance 샘플링만 없다. 모니터링·에디터 같은 규칙), 타일 LOD 임계는 2.5 device px(`TERRAIN_LOD_THRESHOLD_PX`, 컨텍스트 지형에만 LOD 가 있다), 바다 파도 감쇠는 1500/5000m 로 셰이더가 그 밖 픽셀의 파도 계산을 건너뛴다. 성능 확인은 `localStorage crane:perf-hud='1'` HUD(드로우콜·삼각형·프레임 ms·fps — demand 루프라 fps 는 실제로 그린 빈도). **바다 평면은 불투명 패스 뒤(renderOrder 0.25)에 스텐실로 그린다**(2026-09-12): 모든 GLTF 인스턴스 메시 머티리얼이 `markSceneOpaqueStencil`(`packages/domain/src/3d/lib/scene-stencil.ts`, `useClonedModel` 이 원본에 켬, clone 이 물려받음)로 "불투명 씬이 그려졌다" 비트(0x02)를 ZPass 에 찍고, 바다(`sea-surface-material.ts`)는 depthTest/depthWrite 없이 그 비트가 없는 픽셀에서만 그려진다 — 지도·드라이독·잠긴 선체가 깊이 순서와 무관하게 바다 위에 남는 옛 규칙 그대로이면서, 가려진 픽셀은 early stencil 로 셰이더 앞에서 탈락해 파도 노이즈(픽셀당 50여 회)가 야드 아래에서 돌지 않는다(예전엔 바다를 맨 먼저 그려 화면 전체가 매 프레임 파도 계산이었다). 실루엣 마스크·헐은 자기 비트(0x01)만 writeMask/funcMask 로 본다. **규칙**: 바다 위에 보여야 하는 불투명 오버레이(선택 박스·텍스트 테두리·격자·가드 링)는 renderOrder ≥ 0.5 여야 한다(0 이면 물 위에서 바다에 덮인다). 바다 셰이더에 `gl_FragDepth`(logdepthbuf 청크)를 다시 넣거나 depthTest 를 켜면 early test 가 꺼져 절감이 사라진다. `useClonedModel` 을 거치지 않는 GLTF 로드 경로(collision-guard 감지 객체 등)는 transparent 머티리얼이라 바다 뒤에 블렌딩돼 무관하지만, 새 **불투명** 경로를 추가하면 같은 표식을 켜야 물 위에서 사라지지 않는다. **reverse-Z 는 후보에서 뺐다**: three r183 의 `reverseDepthBuffer` 는 WebGPURenderer 전용이고, 기본 프레임버퍼(24bit unorm 깊이)에선 부호 반전만으로 정밀도가 늘지 않아(float 깊이 RT 로 돌려야 이득) 로그 깊이의 z-fighting 해결을 대체할 수 없다. 남은 후보: 타워크레인 데시메이션/LOD, 지도 KTX2, 바다만 애니메이션인 유휴(sea-only)를 30fps 아래로 낮추는 거버너 단계.
...
- GLB 자산은 압축본만 `apps/shell/public/{models,maps}/` 에 배포되고, **압축 전 원본은 `assets-src/` 에 보관**한다. 압축은 되돌릴 수 없으므로 이 디렉토리를 지우지 않는다.
  ...
  - **모델 LOD**(2026-09-11): `node scripts/add-model-lod.mjs <파일>` 이 assets-src 의 현재본에 거리별 LOD 체인(LOD1 35%·LOD2 12%·LOD3 5%, 상대 오차 상한 0.5/2/5%)을 형제 노드로 붙인다(extras `{lodGroup, lod, lodError}`, 지형 타일 LOD 와 같은 계약이라 런타임 `SceneTerrainLod` 가 그대로 전환 — 모델 그룹은 매 프레임 LOD0 월드 위치로 거리를 재 주행하는 크레인도 따라간다). 기존 노드 뒤에만 붙여 `[index]name` 메쉬 경로가 변하지 않고, 정점 accessor 는 공유·인덱스만 추가라 파일 증가는 작다. **meshOverrides·내부 노드 tagMappings·rigId·skin 이 있는 모델은 스크립트가 거부한다**(LOD 사본이 구동을 못 따라감 — goliath_crane 이 그 예). 노드당 5,000 삼각형 미만은 대상 아님(Block). 이어서 `pnpm optimize:glb <파일>` 필수. LOD 전 원본은 `assets-src/models/<파일>.nolod`(자산 교체 시 함께 지운다). 적용됨: TTC-27·TTC-28·TTC-K5000·R370·gantry_crane·crane·hanwha-ocean-lngc-174k. `pnpm perf:scene` 은 LOD>0 노드를 렌더 집계에서 뺀다. 실루엣 테두리·BVH·raycast·충돌은 LOD0 만 본다(`terrainLodProxy` 표식).
...
- **태그 맵핑(`tagMappings[]`)** 은 "서버(PLC) 태그 값 하나 → 트랜스폼 채널 하나" 의 목록이다(모델 인스턴스 필드). 대상은 `{kind:'node', node, channel:position|rotation|scale, axis}`(`node: ''` = 모델 루트) 또는 `{kind:'joint', jointId}`(할당된 리그의 관절) 이고, 태그는 `tagKey` 문자열(`${craneId}:${tagCode}` 공간)로만 참조한다. 적용 공식은 `offset + value × scale` 을 **rest 기준 Δ** 로 더하는 것 — 루트의 rest 는 씬 배치 transform, 내부 노드는 GLTF rest 다. 같은 대상 중복은 sanitize 가 첫 항목만 남기고(first-wins), 리그 관절이 점유한 노드·축은 드라이버가 관절을 우선한다. UI 는 둘 다 amber 로 경고한다(`tag-mapping-editor.ts`).
  ...
  - 값 흐름: 생산자(가상 태그 러너 / WebSocket 러너 / 리플레이) → `publishTagValue(key, v)` (`tag-value-bus.ts`, 표시용 `tagLiveValues` 캐시) → `useTagBindingSource` 가 건 `createTagBindingSource(resolve)` → `rigValueStore`(smooth) → `useRigDriver`(Canvas 안 `RigDriver`) 가 매 프레임 노드별로 rest 로 되돌린 뒤 채널 Δ 를 누적(`apply-channel.ts`). 바인딩은 모니터링 뷰·에디터 모두 화면이 떠 있는 동안 항상 켜 두고 언마운트 시 값 저장소를 비운다. 재생 토글(가상 태그 관리 페이지·플레이백 트랜스포트 바)은 러너 틱만 켜고 끄므로 일시정지하면 노드가 마지막 값에 머물고, 초기값 복귀는 리셋 버튼이 한다. 에디터에는 시뮬레이션 UI 가 없다(팔레트 "시뮬레이션" 탭은 2026-09-17 에 뺐다 — 진입·이탈의 `stopSimulation()` 은 관리 페이지에서 켜진 러너가 넘어오지 않게 하는 방어). 기즈모 드래그 중엔 루트 맵핑을 건너뛴다.
  - **시뮬레이션 시계·시나리오**(2026-09-12): 러너(`virtual-tag-runner.ts`)는 벽시계 dt × `speed`(스토어 세션값, 0.1~16, 선택지 `SIMULATION_SPEED_OPTIONS`)를 누적하고 `seek(ms)` 로 경과를 옮겨 모든 값을 그 시각으로 다시 계산해 내보낸다(정지 중에도 — `attachConfig` 로 getter 만 붙여 둔다). **시나리오**(`VirtualScenario`: id·name·loop·tracks[key, keyframes[atMs,value,ease?]], 세트 `scenarios[]` 에 태그와 함께 저장, `sanitizeScenario*`·`normalizeKeyframes` 가 정렬·중복 제거·상한)는 `activeScenarioId`(세션)로 하나만 활성이고, 트랙이 있는 태그는 `evaluateScenarioTrack`(첫/마지막 값 유지, 구간 ease linear/hold/smooth)이 파형을 대신하며 트랙 없는 태그는 파형 그대로다. loop 아닌 시나리오가 끝에 닿으면 러너가 `onFinished` → 스토어 `pause()`. 시나리오 선택은 `seek(0)`. **종료(관제 복귀)** `model/stop-simulation.ts` 의 `stopSimulation()`(패널·배지·화면 진입/이탈이 전부 이걸 부른다) = 충돌 세션 기록·정지 상태 `clearHistory()` + 스토어 `stop()`: 러너 정지·시간 0(publish 없는 `resetValues(false)`)·시나리오 해제 + `rigValueStore.reset()`(rest 복귀)·`tagLiveValues.clear()`(운전 상태 unknown)·실시간 `release()` — 일시정지(자세 유지)와 다른 경로, 패널의 ■ 버튼(모니터링은 `onStop` 으로 카메라 원래 위치·포커스 해제까지 = 처음 화면). 모니터링 `useSceneData` 와 에디터 페이지는 **진입·이탈 모두** `stop()` 을 불러 화면마다 깨끗한 시뮬레이션으로 시작한다. 러너 `start()` 는 현재 상태값만 내보내고 시각 기준 재계산(evaluateAll)을 하지 않는다 — 한계로 뒤처진 값이 재개 순간 목표로 점프해 미끄러지던 버그(2026-09-12). UI: 공용 패널 `features/3d/ui/scene-simulation-panel.tsx`(재생·리셋·배속·시나리오·반복·스크럽)를 플레이백 트랜스포트 바 팝오버가 쓰고(독 팝업 `scene-simulation-menu.tsx`·에디터 팔레트 태그 탭은 2026-09-17 에 뺐다), 편집은 관리 페이지의 `widgets/virtual-tags/ui/scenario-section.tsx`(목록·이름·반복·실행, 트랙별 키프레임 표(초 단위 입력·태그 단위 값·보간)·SVG 미리보기). HUD 연결 칸 라벨에 `×배속 mm:ss`(`lib/sim-clock.ts`). **시뮬레이션 중 표시** `ui/scene-simulation-badge.tsx`(스토어 `hasSession` — start 에 true, stop 에 false, 일시정지 중에도 true): 캔버스 가장자리 하늘색 inset 테두리(재생 진하게·정지 흐리게) + 좌측 상단 배지(맥동 점·시나리오 이름·배속·■ 종료). `Monitoring3dView` 좌측 상단 열 첫 항목, `toolbarLayout='none'` 제외. 배포 세트에 필리 데모 시나리오 1개(`scenario-philly-block-demo`, 90초)
  - **속도·가속 한계**(2026-09-12): `VirtualTagDefinition.limits?: {maxSpeed?, maxAccel?}`(태그 단위/초·초², 양수만, 관리 표의 두 열). 러너가 파형·시나리오 **목표값**을 향해 `rateLimitSteps`(`@crane/domain/virtual-tag` `lib/rate-limit.ts` — `rateLimitStep`(속도 상한, 가속 상한 + 정지 거리 √(2a·d) 상한, 오버슈트 클램프, 상태에 `velocity`)을 틱 길이(sim 100ms) 이하 서브스텝으로 반복)으로 램프한다. dt 는 배속을 곱한 시뮬레이션 시간이라 ×8 이어도 한계는 시뮬레이션 초 기준이고, 서브스텝 덕에 가속 프로파일도 배속과 무관하다(한 스텝에 0.8s 를 넣으면 프로파일이 1~2틱으로 붕괴해 stop‑go 히치가 났다, 2026-09-16). manual 슬라이더·seek·리셋은 즉시(텔레포트). 배포 필리 태그 7개에 한계가 들어 있어 sine 파형은 한계에 잘려 평평해진다(의도 — 순간이동 방지)
  - **가상 태그**(`@crane/domain/virtual-tag`)는 서버 없이 태그 값을 만드는 정의다(`key`·범위·`initial`·`pattern`: manual|triangle|sine|sawtooth|square). 전역 스토어 `useVirtualTagStore` 가 들고 있으며 영속화는 씬과 같은 규칙(`virtual-tag-storage.ts`)이다 — dev 는 `POST /__dev/virtual-tags` 로 배포 파일 `apps/shell/public/simulation/virtual-tags.json` 에 기록(커밋해 클론·배포의 기준값), 운영은 localStorage `crane:virtual-tags` 에 배포 해시 도장(`baseVersion`)과 함께 봉투로 저장하고 새 배포가 나오면 배포본이 이긴다. 편집은 메모리에만 쌓이고 페이지의 저장 버튼(`save()`)이 기록한다(dirty·미저장 이탈 경고는 씬 편집과 같은 훅 재사용). `virtualTagRuntime`(모듈 전역 `setInterval`, Canvas 불필요)이 틱마다 버스로 내보낸다. 모니터링 `mode='simulation'` 이 이 재생을 켠다(`useSceneData` 의 `autoStartSimulation`, 기본 true. 대시보드 3D 미리보기 모달은 ▶ 토글이 없는 뷰라 false 로 정지 상태로 연다). 실시간 페이지(`mode='realtime'`)에는 시뮬레이션 UI 가 없고, 플레이백(`mode='playback'`, 소스 시뮬레이션)은 상단 트랜스포트 바 ▶ 가 켠다(자동 재생 없음). 콤보박스 목록은 `useTagCatalog` 하나를 본다 — 실서버가 붙으면 여기에 `getMonitoringTags()` 결과를 `source:'server'` 로 합치고 WebSocket 러너를 켜면 되며, 씬 JSON·맵핑 UI·드라이버는 무변경이다.
