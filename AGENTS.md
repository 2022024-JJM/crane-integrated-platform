...
- **모니터링·리플레이 캔버스는 `frameloop='demand'` 이고 프레임은 `SceneFrameGovernor`(`packages/features/src/3d/ui/scene-frame-governor.tsx`, 판정 `lib/frame-governor.ts`)가 만든다** (2026-09-11 유휴 발열 절감). 애니메이션 소스(가상 태그 재생, 실시간은 최근 1.5초 안에 메시지를 받은 경우만(`useRealtimeStore.activity.lastMessageAt`), 리플레이 재생, 기즈모 드래그, 바다 EXR 씬)가 있으면 30fps 틱, 멈춘 뒤 1.5초 유예(스무딩 정착), solar 태양만 있으면 5초에 한 번, 정지 씬은 틱 없음 — 조작은 OrbitControls·표면 카메라가 스스로 invalidate 해 주사율로 그려진다. **씬을 매 프레임 바꾸는 새 경로를 추가하면 거버너의 소스 목록에 넣거나 스스로 `invalidate()` 해야 한다** — 안 하면 화면이 다음 조작까지 낡은 프레임에 머문다(model-mesh 의 머티리얼 effect 가 선례). 에디터 캔버스는 아직 'always' 다. 값 저장소의 스무딩 그림자 무효화는 임계 2cm·0.02° 에 초당 20회 상한(`rig-value-store.ts` `SHADOW_STEP_*`)이다. **절감은 핵심(야드 지도 kind 'ground'·크레인)이 아니라 주변에서 한다** — DPR 상한은 1.5 그대로(`SCENE_DEFAULT_DPR` + `three-scene-viewer.tsx` 기본값, 함께 바꿀 것. 1.25 로 내렸다 되돌림), 컨텍스트 지형(kind 'context')은 `GltfModel shading='lambert'` 로 PBR 대신 Lambert(`packages/domain/src/3d/lib/lambert-material.ts`, 원본 머티리얼당 변환본 캐시 — 조명·낮/밤엔 똑같이 반응하고 스펙큘러·radiance 샘플링만 없다. 모니터링·에디터 같은 규칙), 타일 LOD 임계는 2.5 device px(`TERRAIN_LOD_THRESHOLD_PX`, 컨텍스트 지형에만 LOD 가 있다), 바다 파도 감쇠는 1500/5000m 로 셰이더가 그 밖 픽셀의 파도 계산을 건너뛴다. 성능 확인은 `localStorage crane:perf-hud='1'` HUD(드로우콜·삼각형·프레임 ms·fps — demand 루프라 fps 는 실제로 그린 빈도). 남은 후보: reverse-Z 깊이(`WebGLRenderer.reverseDepthBuffer`, early-Z 복원)로 로그 깊이 대체 + 바다를 불투명 뒤에 그리기, 타워크레인 데시메이션/LOD, 지도 KTX2.
