...
| 씬 객체 충돌 감지(시뮬레이션·실시간 모니터링과 에디터, 모델↔모델 관통 감지. 골리앗 LiDAR 근접 존인 collision guard 와는 별개) | 기하 프리미티브 `packages/domain/src/3d/lib/collision-volumes.ts`(AABB→OBB→three-mesh-bvh 삼각형, BVH 는 여기서만 접근·빌드 안 함. 실루엣 테두리는 대상 메시의 **자식**으로 포털되므로 `lib/overlay-mesh.ts` 표식으로 수집에서 뺀다 — 없으면 마스크가 대상 지오메트리를 재사용해 같은 형상이 두 번 잡히고 헐은 BVH 가 없어 영영 판정 불가를 답한다), **판정 캐스케이드** `packages/features/src/3d/lib/scene-collision-probe.ts`(모델 AABB→메쉬 AABB→OBB→삼각형과 억제 해제용 메쉬 분리 판정. 감지와 예측이 **반드시 공유**한다 — 각자 구현하면 "예측은 났는데 감지는 없다" 가 된다), 런타임 `packages/features/src/3d/model/scene-collision-runtime.ts`(React 밖 싱글턴, 스케줄링 전담: 메쉬 matrixWorld 변화 기반 dirty 쌍만 검사, 기준선·억제·시간 예산. hit 을 돌려줄 뿐 스스로 멈추지 않는다), 스토어 `model/use-scene-collision-store.ts`(세션 전용 전역: on/off·충돌 시 정지 여부 — 둘 다 기본 ON(2026-09-08, 새로고침 시 다시 ON)·기록 최대 10개·활성 기록. 기록 클릭 = `rigValueStore.restore(스냅샷)` + 러너 정지, ▶ 재생이 `resume` 으로 재무장), 훅 `model/use-scene-collision-detector.ts`(**`RigDriver` 바로 다음에 마운트** — 같은 priority useFrame 은 마운트 순. **감지는 `runner` 가 재생 중(+기즈모 드래그 아님)일 때만 돈다**(2026-09-08, `scene-collision-hold.ts` 의 `isRunnerRunning` — 시뮬레이션은 가상 태그 `isRunning`, 실시간은 진입~이탈 내내 재생이라 `held` 여도 스캔). 스캔 재개 전이·기록 복원(`selectRecord`)은 `rebaseline()`, 새 모델 항목(첫 마운트·리마운트·참조 교체)은 런타임 tick 이 스스로 기준선으로 되돌리고, 기준선은 큐가 빈 뒤에도 `BASELINE_SETTLE_MS`(1s) 안정화 창이 지나야 scanning 이 된다 — 정지 중 기즈모·인스펙터·슬라이더로 만든 겹침, 드래그로 놓은 겹침, 로딩 배치, 재생 시작 스무딩은 보고 대신 억제되고 시뮬레이션이 떼었다 다시 붙여야 보고된다. 창 안에서 시뮬레이션이 만든 겹침도 분리 전까지 보고되지 않으며, 재생 중 편집(opacity 만 바꿔도 참조 교체)마다 1초 blind 가 생기는 것은 허용된 부작용이다. 어느 모드든 그 쌍만 억제한 채 감시 계속 — 정지 모드는 러너 pause+freeze+pin(스캔도 멈춤), 무정지는 3초 박스. 억제 해제는 모델 AABB 가 아니라 **메쉬 단위**(AABB+margin → OBB → 삼각형 최단 거리 `meshesWithinDistance`)라, 단일 메쉬 크레인처럼 OBB 가 늘 겹치는 모델도 삼각형이 0.05 unit 이상 떨어지면 재보고된다), 편집 UI 는 `packages/features/src/3d/ui/scene-collision-panel.tsx`(감지 on/off·충돌 시 정지·충돌 예측 on/off·예측 범위·기록 10개·초기화) 하나를 에디터 팔레트 "충돌" 탭과 모니터링 독 `AlertTriangle` 팝업 `ui/scene-collision-menu.tsx`(3단계 우선순위 red 충돌 > mint 예측 > amber 감지 켜짐 — variant 를 하나 계산해 한 번만 적용한다, 조건부 클래스를 나열하면 border/bg/text 가 두 벌 붙어 순서로 결정된다)이 공유한다(충돌 시 캔버스 위 오버레이 패널은 2026-09-07 에 제거. 화면 수준 경보는 `ui/scene-collision-alert-overlay.tsx` — 가장자리 inset box-shadow 비네트 + 상단 배너, 아이콘·독 버튼은 깜빡이지 않고 실시간 정지도 제목은 "충돌 감지". 배너의 재개 버튼 "이어서 재생"은 두 모드 모두 가상 태그 러너 재생(독 ▶ 와 동일, 2026-09-08 — 실시간 페이지에서도 값은 독 ▶ 가 켠 시뮬레이션이 만들어, 보류만 풀던 옛 "최신 값 복귀"는 아무것도 움직이지 않았다) — 와 씬 안 충돌 장비 일체형 빨간 실루엣 테두리(`@crane/domain/3d` 의 `ObjectSilhouetteOutline` — 스텐실 마스크 + 인플레이션 헐 `lib/silhouette-outline.ts`, `SCENE_GL_OPTIONS.stencil: true` 필요. 에디터의 선택 표시도 같은 컴포넌트를 노랑으로 쓴다 — 모델 전체와 안쪽 노드(계층 목록·drill-in) 모두: `GltfModel` 의 `selectionStyle='outline'`, 스텐실 없는 캔버스에선 기본 'box' 유지. 에디터의 **지도**는 'box' — 수 km 지형을 실루엣으로 두르면 화면 가장자리 전체가 테두리가 되고 헐이 지형 메시를 매 프레임 두 번 더 그린다)·경고 표지 `scene-collision-highlight.tsx`). 모니터링은 시뮬레이션·실시간 모드에서 켜지고 리플레이는 제외. 값 생산자 정지는 `model/scene-collision-hold.ts` 한 곳 — 가상 태그는 pause, 실시간(WebSocket)은 `useRealtimeStore.hold()` 로 **화면 반영만 보류**(러너가 수신 값을 drain 만 하고 버림, 장비는 계속 움직임)하며 pinned 을 떠나는 모든 스토어 경로(resume·clearActive·clearHistory·clear·setEnabled(false))가 release 한다. 패널은 `runner='realtime'` 이면 실시간 문구를 보이고, 정지 해제는 고정된 기록 행 재클릭(`selectRecord` → `resume`)이다(독 ▶ 도 풀지만 실시간에선 가상 태그 러너를 켜는 부작용이 있어 안내하지 않는다). 상수 `lib/scene-collision-pairs.ts` |
| 씬 객체 충돌 **예측**(감지 하위 토글, 가상 태그가 장비를 움직이는 동안) | 값 `packages/features/src/3d/lib/tag-prediction.ts`(`sampleFutureTagValues` — `stepVirtualTag` 이 `elapsedMs` 만의 순수 함수라 경과 시간을 밀어 넣으면 N초 뒤 태그 값이 **정확히** 나온다. manual 패턴은 상태를 그대로 돌려줘 미래값=현재값. `buildPredictedAddressValues` 가 맵핑 인덱스로 주소별 `offset + v*scale` 을 만들고 공식은 라이브와 같은 `applyTagBinding`(rig-value-store) 하나를 쓴다), 자세 차용 `model/use-rig-driver.ts` 의 `rigPoseBorrow`(**사본을 만들지 않는다** — 실제 노드 트리에 미래 값을 적용해 검사하고 같은 콜백에서 되돌린다. clone 을 피한 이유는 `getRestPose` 가 clone 직후 seed 되는 WeakMap 이라 구동된 트리를 다시 clone 하면 구동 자세가 rest 로 굳고, `accumulatedParentScale` 이 라이브 부모 체인을 걷고, registry 가 같은 id 를 덮어쓰고, 무엇보다 **루트 rest 가 기즈모 종료 프레임에 다시 잡히기** 때문이다. `borrow`/`release` 는 try/finally 로 짝을 맞추고 `release` 는 로컬 복원 뒤 `updateMatrixWorld(true)` 를 **반드시** 부른다 — three 는 force 경로에서 `matrixWorldNeedsUpdate` 를 내리고 `matrixAutoUpdate=false` 인 노드는 `matrix` 재합성을 건너뛴다. **`invalidateShadows()` 는 부르지 않는다** — 같은 프레임에 되돌려 화면이 안 바뀌는, 그림자 무효화 규칙의 유일한 예외다), 검사 `model/scene-collision-prediction-runtime.ts`(무상태. 기준선·억제·dirty 를 재사용하지 **않는다** — 안정화 창이 예측을 1초 억제하고 억제 집합이 첫 발견 뒤 침묵시킨다. 쌍은 구동 모델 × 전체(정적↔정적 제외)에서 `sceneCollisionRuntime.suppressedKeys` 와 활성 기록 쌍을 뺀 것 — 빼지 않으면 에디터에서 겹쳐 둔 모델이 영구 민트 경보가 되고, 충돌 시 정지를 끄면 같은 쌍에 빨강·민트가 함께 뜬다. `no-bvh` 는 재시도 없이 넘긴다. 예측이 꺼질 때 부르는 `reset` 은 항목·쌍 캐시만 비우고 **씬 모델 목록은 남긴다** — 목록을 채우는 `sync` 는 `models` 참조가 바뀔 때만 불리므로, 지우면 감지를 껐다 켜도 목록이 빈 채로 남아 예측이 영영 안 뜬다(실측 2026-09-10). 감지 런타임의 `disarm` 도 같은 이유로 `models` 를 건드리지 않는다), 훅 `model/use-scene-collision-prediction.ts`(**`SceneCollisionDetector` 직후 마운트** — 앞서면 감지의 변화 감지가 미래 행렬을 움직임으로 읽어 거짓 충돌을 기록·정지까지 보고한다. 게이트는 감지 on + 예측 on + **가상 태그 러너 재생 중** + 기즈모 드래그 아님이고 닫히면 `predicted` 를 비운다. 페이지 `mode` 가 아니라 러너 상태를 보는 이유: 실시간 모니터링 화면도 독 ▶ 로 가상 태그를 켜면 그것이 장비를 움직이므로 예측이 그대로 성립하고(AGENTS.md 감지 항목의 "실시간 페이지에서도 값은 독 ▶ 가 켠 시뮬레이션이 만든다"), 실제 WebSocket 값만 흐를 때는 러너가 꺼져 있어 저절로 빠진다. 그래서 마운트는 감지와 같은 범위(리플레이만 제외)이고 판정은 훅 하나에 모인다 — 충돌 정지로 러너가 멈추면 민트가 저절로 사라진다. 사다리는 **단일 시간 원점 + 격자 정렬**: 스윕 시작에 `virtualTagRuntime.elapsed` 를 한 번만 읽고 `alignSweepBase` 로 250ms 격자에 내림한 뒤 칸 k 목표를 `base + k×250ms` 로 잡아 `leadTimeSec = 목표 − 현재 경과`. 격자 정렬이 없으면 원점이 스윕마다 100ms 씩 흘러 칸 시각이 통째로 움직이고, 충돌 시각이 250ms 안쪽으로 붙은 두 쌍의 **검출 순서가 뒤집혀 예측 표시가 번갈아 뜬다**(실측 2026-09-10). 같은 칸에서 여러 쌍이 겹치는 경우는 `scanSample` 의 `preferPairKey` 가 지금 띄우고 있는 쌍을 유지한다. 칸마다 다시 읽으면 도는 동안 시간이 흘러 빗살이 미끄러지고 리드타임이 과소 보고된다. 오름차순이라 첫 hit 이 최단이고 거기서 중단, 예산 초과는 칸 커서로 다음 틱이 같은 원점을 유지한 채 이어받는다. 스로틀은 프레임 수가 아니라 `performance.now()` — `frameloop='demand'` 대응), 표시 `ui/scene-collision-prediction-highlight.tsx`(두 겹 — (1) **고스트**: 부딪히는 순간의 자세로 같은 GLB 를 한 벌 더 그린다(`@crane/domain/3d` 의 `GhostModel` + 프레넬 림 `lib/hologram-material.ts`, opacity 별 머티리얼 캐시로 재컴파일 방지). 실물 트리를 재사용하지 않고 새로 clone 해 자세 스냅샷(`lib/ghost-pose.ts` 의 로컬 transform)을 경로로 덮어쓰므로 리그 드라이버·rest 캐시와 얽히지 않는다. **구동 모델만** 그린다 — 정적 장비는 미래가 현재와 같아 이중상만 된다. **실물이 씬 루트 직계이고 고스트도 같은 층이라는 전제**(로컬 transform 스냅샷)라 모델을 변환 group 으로 감싸게 되면 고스트도 그 안에 넣어야 한다. (2) **카운트다운**: 접촉 예상 지점 **위**에 띄우고 DOM 리더선으로 잇는다 — 접촉점에 겹치면 정작 보여 줄 고스트를 가린다. 리더선은 블록의 마지막 자식이고 블록 전체를 자기 높이만큼 올려(`-translate-y-full`) 선 끝이 접촉점에 닿는다(띄우는 높이 = 선 길이라 값이 한 곳). 블록이 화면 픽셀로 떠 있으므로 이어 주는 선도 DOM 이어야 줌과 무관하게 길이가 맞는다. 원 안은 "충돌까지"·숫자·단위 3줄이고 남은 비율 호가 함께 줄어든다(`countdownArc`). 2026-09-10 에 세 가지를 폐기했다 — 바운딩 박스(형태를 잃어 어느 부품이 어디로 가는지 안 읽힘), 중간 시점 잔상 고스트(화면을 겹겹이 덮기만 함), 접근 궤적 점선·지면 핀(고스트가 이미 "어디로 가는지" 를 형태로 말하는데 선이 하나 더 깔리면 시선만 분산됨). 전부 **실루엣·스텐실을 쓰지 않는다** — 참조값이 캔버스당 하나라 빨강과 동시에 뜨면 발자국이 전역 합집합이 되고, 실루엣은 애초에 현재 위치만 두를 수 있다. **홀로그램 셰이더는 `logdepthbuf` 청크를 반드시 include 한다** — 씬이 `logarithmicDepthBuffer: true` 라 raw ShaderMaterial 이 표준 z 를 쓰면 깊이 비교가 어긋나 고스트가 카메라 거리·각도에 따라 통째로 사라진다(실측 2026-09-10, 실루엣의 옛 inverted hull 이 탈락했던 것과 같은 함정). 블렌딩은 `NormalBlending` — additive 는 밝은 지형·하늘 위에서 포화돼 사라진다. **시간 변조(맥동)를 두지 않는다** — 임박할수록 빠르게 뛰게 했더니(최대 2.6Hz·진폭 0.45) 그냥 번쩍이는 것으로 보였다(2026-09-10). 긴박함은 카운트다운이 맡는다. 고스트 메시는 `HOLOGRAM_RENDER_ORDER` 로 정렬을 고정한다 — 투명 객체를 깊이로 정렬하는데 크레인처럼 메시 수십 개가 비슷한 거리에 있으면 카메라를 돌릴 때 순서가 뒤바뀌며 블렌딩이 튄다. 수치는 `lib/prediction-visual.ts`)·`ui/scene-collision-prediction-overlay.tsx`(상단 배너, **모니터링에만**. `activeMode !== null` 이면 스스로 내려가 실제 충돌이 이긴다. **가장자리 비네트는 두지 않는다**(2026-09-10 제거) — 실제 충돌은 즉시 개입해야 하는 일회성 사건이라 화면을 물들일 값어치가 있지만 예측은 재생 내내 떴다 사라지는 지속 상태라, 배경을 계속 물들이면 정작 봐야 할 고스트가 가려지고 빨간 충돌 비네트와 겹치면 어느 쪽도 안 읽힌다. `aria-live='polite'` — 리드타임이 계속 갱신된다). 스토어는 감지와 같은 `use-scene-collision-store.ts`(`predictionEnabled` 기본 ON·`predictionHorizonSec` 기본 10초·`predicted`. 고스트 자세·접촉점은 쌍 첫 발견 시점에 **고정**하고 리드타임만 0.1초 단위로 갱신 — 캡처가 자세 차용이 드는 경로이고, 매 스윕 갱신하면 clone 이 계속 다시 만들어진다. `initialLeadTimeSec` 이 카운트다운 호의 분모다. 비우는 경로 5개: `setEnabled(false)`·`setPredictionEnabled(false)`·`clear`·`clearHistory`·`selectRecord`). **정확도 한계 3개는 보정하지 않고 문서화한다** — (1) 점 샘플링이라 칸 사이를 통과하는 충돌은 놓친다(실측 최대 `LLC_002:slew_angle` 28.3°/s, 반경 50 붐 끝이 250ms 에 약 6 unit. 1초 칸이면 25~30 unit 을 건너뛰어 대표 시나리오가 전부 안 보여 칸 간격을 고정했다), (2) 예측은 태그 target 을 적용하지만 화면은 SmoothDamp(0.35s)를 거쳐 리드타임이 약 0.35초 이르다(안전한 방향), (3) square 파형은 불연속 점프라 미래 smoothed 값을 모른다. 예측은 경보만 하고 러너를 멈추지 않는다. 리플레이는 미래 프레임을 이미 들고 있어 "예측" 이 아니라 정확한 사후 계산이므로 섞지 않는다. 상수 `lib/scene-collision-pairs.ts`. 2026-09-10 도입 |
| 카메라 이동 범위 제한(모니터링·리플레이·에디터 공용) | 적용 `packages/features/src/3d/ui/scene-camera-limits.tsx`(`SceneCameraLimits`, `SceneSurfaceCamera` **바로 다음 형제**에 마운트 — 같은 priority useFrame 은 마운트 순), 수식 `lib/camera-limits.ts`(테스트 대상). 바닥은 **카메라 아래 지도 표면**(씬의 모든 지도에 `raycastMapSurfaceY`, 카메라 XZ 가 0.5m 움직일 때만 재측정, 히트 없으면 `SEA_LEVEL_Y`) + 5m 로 EXR 유무 무관(옛 `CameraAboveSea` 흡수) — 회전은 `maxPolarAngle` 을 매 프레임 갱신(바닥 기준 동적 상한과 고정 75° 중 작은 쪽)해 막는다. 이동 범위는 **타깃이 아니라 카메라 XZ** 를 기준 지도들의 월드 AABB 합집합 안으로 제한 — 기준 지도는 지도 인스펙터 **카메라 탭**의 "카메라 영역 제한" 체크(`SavedMapInfo.cameraBounds`, 옵트인·true 만 저장)가 켜진 것들이고 하나도 없으면 씬의 모든 지도(`resolveCameraBoundsMaps`/`unionObjectBounds`/`collectCameraBoundsBox`, `packages/domain/src/3d/lib/camera-bounds-maps.ts`). 팔레트로 ground 지도를 추가하면 `addSceneMap` 이 체크된 채 넣고, `resolveGroundMap` 은 드롭 raycast 바닥면에만 남는다. 뷰어·리플레이 `getTopViewBounds` 와 에디터 `topView` 도 같은 합집합을 본다(표면 피벗이 타깃을 지도 밖 지형·바다에 놓으므로 타깃 기준이면 드래그마다 튄다. 여유·여백 비율은 두지 않는다 — 지도가 작업 구역보다 훨씬 넓은 philly 조선소에서 바깥 여유는 무의미하고 안쪽 여백은 가장자리 모델을 잘라냈다). 위반 처리는 되밀기가 아니라 **그 프레임의 평행이동 취소**(카메라·타깃 델타가 같으면 팬·dolly → update 직전 자세로 복원, 경계에서 그냥 멈춤)이고, 그래도 밖이면(회전·프레임 밖 명령·로드 직후) 경계로 투영한다. `controls.maxDistance` 는 그 합집합의 탑뷰 fit 거리 × 1.0(탑뷰가 곧 가장 먼 시점, 상한 `CAMERA_MAX_DISTANCE` 30000 — 2026-09-09 에 3000 에서 올림, 지도 없으면 30000)로 여기서 정하고 휠 dolly 상한·표면 피벗 거리 cap(`placePivot`, 밖 지형을 볼 때 반경 clamp 로 튀는 것 방지)·뷰어와 에디터의 탑뷰 상한(반높이 차감)이 같은 값을 읽는다. 제한 뒤 `controls.update()` 금지(damping 이중 적용), `change` 이벤트만 발행해 에디터 카메라 상태가 따라온다. `minPolarAngle` 은 0(정수직 탑뷰 허용). 2026-09-09 도입 |
...
- GLB 자산은 압축본만 `apps/shell/public/{models,maps}/` 에 배포되고, **압축 전 원본은 `assets-src/` 에 보관**한다. 압축은 되돌릴 수 없으므로 이 디렉토리를 지우지 않는다.
  ...
  - **배포된 philly-terrain 은 8×8 공간 타일 + LOD 체인 구조다** (2026-09-09, `node scripts/tile-terrain-glb.mjs --lod` 산출물 — 45타일 × LOD0~3(오차 0/2/4/8m) 형제 노드, extras `{tile,lod,lodError}`. 무텍스처 머티리얼 28개는 COLOR_0 unorm16 정점색으로 1개 병합, LOD0 은 삼각형·bbox 완전 보존, LOD1~3 은 정점 accessor 를 LOD0 과 공유하고 인덱스만 별도). 단일 노드 시절엔 frustum 컬링이 전무해 어느 방위든 씬 251만 tris 전량 렌더였고, 타일+LOD 후 83만~157만(실측, worst 다운타운 정면 −38%)이다. 런타임 전환은 `@crane/features/3d` 의 `SceneTerrainLod`(스크린 오차 1 device px 미만 레벨 선택, 수식은 `lib/terrain-lod.ts`) — 세 캔버스에 마운트돼 있고, LOD>0 노드는 clone 시점에 숨김+raycast 제외된다(model-mesh.tsx — **최상위 캐리어만** 끈다. GLTFLoader 가 extras 를 다중 프리미티브의 자식 Mesh 에도 복제하므로 자식까지 끄면 그룹을 켜도 타일이 사라진다, 실측 결함). raycast·BVH 는 항상 LOD0 담당이라 표면 높이·드롭·클릭은 LOD 상태와 무관하다. **이 파일에 `pnpm optimize:map` 재실행 금지** — 데시메이션·정리 스테이지가 타일·LOD·정점색을 훼손한다. 재반입 순서: 컨플루언스 원본 → `optimize:map` → `tile-terrain-glb --lod`. 다른 대형 컨텍스트 지형도 같은 스크립트를 검토한다.
...
- philly 두 씬(`goliath.json`, `philly-2dock.json`)은 지도가 **2장**이다 — `maps[0]` 조선소(`phillyshipyard.glb`, 드롭 raycast 기준이자 `cameraBounds: true` 로 카메라 범위·탑뷰 기준)와 `maps[1]` 주변 지형(`philly-terrain.glb`, 조선소 자리가 구멍으로 잘린 시 전역 OSM 지형). 에디터 팔레트 "맵" 탭은 타일별 **추가/제거 토글**이라 지도를 여러 장 놓을 수 있고(`addSceneMap` append · 제거는 `deletePlacedMap`, 같은 경로는 한 장), 터레인도 카탈로그(`scene-map-catalog.ts`)에 `kind:'context'` + `defaultPosition`(goliath 기준 오프셋)으로 등록돼 있다. 드롭 raycast 의 기준인 **바닥 지도는 배열 인덱스가 아니라 카탈로그 `kind` 로 판정**한다(`resolveGroundMap`: 첫 `ground`, 없으면 `maps[0]` 폴백). 카메라 이동 범위·탑뷰 bounds 는 kind 가 아니라 씬 데이터 `cameraBounds`(인스펙터 카메라 탭)로 고른다 — 폭 수 km 짜리 컨텍스트 지형이 탑뷰 프레이밍을 잡아먹지 않게 하려는 구분이고, 두 씬 모두 조선소만 체크돼 있다. 타일 상태 파생은 `packages/widgets/src/3d/lib/map-palette-tiles.ts`. 잠긴 지도 타일은 클릭을 무시하고, 계층 목록·타일 자물쇠로 해제한다. 터레인 배치값은 디자이너 Blender 씬의 조선소 V4 오프셋을 보정한 값이라 조선소 지도를 다시 반입해 좌표계가 바뀌면 터레인도 함께 옮긴다.
