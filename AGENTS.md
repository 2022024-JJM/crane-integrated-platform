...
| 씬 객체 충돌 감지(시뮬레이션 전용, 모델↔모델 관통 감지 → 러너 정지·보고. 골리앗 LiDAR 근접 존인 collision guard 와는 별개) | 기하 `packages/domain/src/3d/lib/collision-volumes.ts`(AABB→OBB→three-mesh-bvh 삼각형, BVH 는 여기서만 접근·빌드 안 함), 런타임 `packages/features/src/3d/model/scene-collision-runtime.ts`(React 밖 싱글턴, 메쉬 matrixWorld 변화 기반 dirty 쌍만 검사, 기준선·억제·시간 예산), 상수·카메라 계산 `lib/scene-collision-pairs.ts`, 스토어 `model/use-scene-collision-store.ts`(세션 전용 전역 토글), 훅 `model/use-scene-collision-detector.ts`(**`RigDriver` 바로 다음에 마운트** — 같은 priority useFrame 은 마운트 순), UI `ui/scene-collision-{detector,highlight,overlay,toggle}.tsx`. 값 저장소 `rigValueStore.freeze()` 가 정지 뒤 스무딩 잔여 수렴을 막는다 |
