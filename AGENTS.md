...
- philly 두 씬(`goliath.json`, `philly-2dock.json`)은 지도가 **2장**이다 — `maps[0]` 조선소(`phillyshipyard.glb`, 드롭 raycast·탑뷰 bounds 기준)와 `maps[1]` 주변 지형(`philly-terrain.glb`, 조선소 자리가 구멍으로 잘린 시 전역 OSM 지형). 에디터 팔레트는 `maps[0]` 만 관리하고 타일 클릭이 배열 전체를 교체하므로 터레인은 카탈로그(`scene-map-catalog.ts`)에 넣지 않고 JSON 으로만 둔다. 계층 목록에서 잠금 해제·이동은 된다. 터레인 배치값은 디자이너 Blender 씬의 조선소 V4 오프셋을 보정한 값이라 조선소 지도를 다시 반입해 좌표계가 바뀌면 터레인도 함께 옮긴다.
