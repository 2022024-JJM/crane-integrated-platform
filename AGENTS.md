...
| 관제 요약 HUD(상단 중앙, 2026-09-12 도입) | `packages/features/src/3d/ui/scene-status-hud.tsx` — 현장 시각(`useSceneSunState`, 리플레이는 프레임 시각)·풍속/풍향(`model/use-scene-weather.ts`: `scene-site-geo` 좌표로 open-meteo 10분 주기 — 헤더 `useHeaderWeather` 는 라우트·옥포 독 좌표뿐이라 따로 둠. `@crane/domain/weather` 의 `WeatherSnapshot` 에 `windSpeed`(m/s)·`windDirection` 추가, 응답에 없으면 null)·가동 n/상태 확인 N·두절(0 이면 숨김)·알람 장비 수(페이지가 넘긴 `alarmsByCraneId`, 최고 severity 색). 풍속 권고 단계는 `lib/wind-advisory.ts`(주의 10·중지 15 m/s, 현장 규정에 맞춰 상수만 조정). pointer-events-none. 독 배치에서만 마운트하고, 같은 상단 중앙을 쓰는 충돌 경보 배너는 `bannerClassName='top-16'` 으로 HUD 아래에 둔다 |
