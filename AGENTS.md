...
- 공통 서브라우트: `3d-monitoring`, `3d-viewer-edit`, `virtual-tags`, `crane-status`, `work-history`, `alarm-history`, `3d-replay`. `goliath-work` 는 여기에 `vision`, `cabin-monitoring` 을 더 가진다. `3d-monitoring` 은 **실시간(WebSocket 만)**, `3d-replay` 는 **플레이백(분석)** 페이지(메뉴 라벨 "3D 플레이백", 2026-09-16 — 기록 리플레이 | 시뮬레이션 소스를 한 3D 뷰에서 재생하고 우측에 실행 리포트. 아래 기준 파일 표의 "플레이백" 행).
...
| 태그 맵핑 편집 UI | `packages/widgets/src/3d/ui/tag-mapping-section.tsx`(인스펙터 탭), `tag-key-combobox.tsx`, `lib/tag-mapping-editor.ts`(충돌 판정·기본값, 테스트 대상), 팔레트 "시뮬레이션" 탭 태그 하위 탭 `palette-virtual-tag-section.tsx` |
...
| 영역 침범 → 알람 계통(2026-09-12) | 로컬 알람 `@crane/domain/alarm` `createLocalAlarm`/`localAlarmActiveKey`(id `local:<type>:<subject>:<at>`(+`:clear`), `eventType 'zone_intrusion'`), 스토어 `useRealtimeAlarmStore.upsertLocalAlarm(alarm, key)`. 브릿지는 **앱 셸** `apps/shell/src/runtime/zone-alarm-bridge.tsx`(features/3d ↔ features/alarm 은 같은 레이어라 서로 import 하지 않는다) — 침범 쌍 생성=발생(stop → high, warn → medium), 소멸=해제. 실시간 화면의 침범만(`filterAcceptedZoneTransitions` — 위 저널 행과 같은 규칙, `openedAt` 이 승인 집합). craneId 는 소유 모델 craneId, 없으면 모델 id 라 지역 필터는 `isAlarmInRegion`(레지스트리 craneId **또는** `alarm.regionId === regionId`)으로 통과 — 알람 슬라이스의 6개 필터 지점이 전부 이 헬퍼를 쓴다 |
...
| 모니터링 씬 독(dock: hover 펼침·고정 우측 레일) | 껍데기 `packages/ui/src/organisms/scene-dock.tsx`(`SceneDockRail`·`SceneDockRailSeparator`, 완전 제어형, 도킹 프레임은 `three-scene-viewer.tsx` 의 `toolbarPlacement="dock"`. 레일 순서는 위에서부터 카메라(원래위치·탑뷰·**저장한 뷰**(`toolbarTrailing`)·확대·축소·전체화면) → 씬 감지(시뮬레이션 ▶·충돌·영역) → 구분선 → 화면 표시(페이지가 준 `toolbarExtras` 알람 토글·골리앗 가드·미니맵·현장 시각). `toolbarTrailing` 을 카메라 묶음 안에 끼우는 것은 dock 배치뿐이고 가로 툴바는 기존대로 앞에 붙는다, 2026-09-12), 상태·영속화 `packages/features/src/3d/model/use-scene-dock.ts` + `lib/{dock-hover-state,dock-storage}.ts`(순수 리듀서·pin 영속화, 테스트 대상). 조립은 `Monitoring3dView` 의 `toolbarLayout="dock"`. 하단 독 패널(크레인 실시간 상태 테이블)은 2026-09-03 에 제거됐다 |
