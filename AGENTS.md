...
| 감지 설정 페이지(메뉴 "설정", 충돌·영역 감지 옵션 5개, 2026-09-17 — 독 팝업·에디터 팔레트 탭에 흩어져 있던 스위치를 한 곳으로) | `packages/widgets/src/detection-settings/ui/detection-settings-page.tsx`(스위치만 — 기록·침범 목록 같은 런타임 상태 없음), 영속화 `packages/features/src/3d/lib/detection-settings-storage.ts`(키 `crane:detection-settings`, 봉투 sanitize, 테스트 대상. 두 스토어가 초기값을 `readDetectionSettings` 로 읽고 setter 마다 `writeDetectionSettings`(read→merge→write) 한다) |
